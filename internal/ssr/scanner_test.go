...
var _ = Describe("Scanner", func() {
	...
	Describe("Stream mode timeout", func() {
		It("RenderPageWithTimeout returns error when stream process stalls", func() {
			// sleep ignores stdin and never writes to stdout — a reliable
			// stall on all platforms. The context timeout must interrupt
			// the blocked read and return an error.
			sr, err := ssr.NewStreamRenderer("sleep 60")
			Expect(err).NotTo(HaveOccurred())
			defer sr.Close()
