...
`
	...
	// ── MarkdownConfig toc toggle (issue #828) ──────────────────────────
	// content.markdown.toc controls whether page.toc is generated.
	// Uses *bool tri-state: nil (omitted) defaults to true, explicit false
	// is preserved. This is an Alloy-level concern (AST walk after parsing),
	// not a goldmark parser option — so it lives on MarkdownConfig, not
	// GoldmarkConfig. Independent of autoHeadingID.

	Describe("MarkdownConfig toc toggle (issue #828)", func() {
		It("omitted toc defaults to true via TOCValue()", func() {
			cfg, err := config.LoadWithDefaults("testdata/minimal.yaml")
			Expect(err).NotTo(HaveOccurred())
			Expect(cfg.Content.Markdown.TOCValue()).To(BeTrue(),
				"TOCValue() must return true when toc is omitted from config — "+
					"nil *bool defaults to true so page.toc is generated by default")
		})

		It("explicit toc: false is preserved after LoadWithDefaults", func() {
			cfg, err := config.LoadWithDefaults("testdata/toc_false.yaml")
			Expect(err).NotTo(HaveOccurred())
			Expect(cfg.Content.Markdown.TOCValue()).To(BeFalse(),
				"TOCValue() must return false when toc is explicitly set to false — "+
					"ApplyDefaults must not overwrite explicit false with default true")
		})

		It("TOC field is non-nil when explicitly set", func() {
			cfg, err := config.LoadWithDefaults("testdata/toc_false.yaml")
			Expect(err).NotTo(HaveOccurred())
			Expect(cfg.Content.Markdown.TOC).NotTo(BeNil(),
				"TOC must be non-nil when explicitly set in config — "+
					"the *bool field must parse the YAML value, not remain nil")
		})

		It("toc is independent of goldmark autoHeadingID", func() {
			cfg, err := config.LoadWithDefaults("testdata/toc_false.yaml")
			Expect(err).NotTo(HaveOccurred())
			Expect(cfg.Content.Markdown.TOCValue()).To(BeFalse(),
				"toc must be false when explicitly disabled")
			Expect(cfg.Content.Markdown.Goldmark.AutoHeadingIDValue()).To(BeTrue(),
