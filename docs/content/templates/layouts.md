...
Each TOC entry has `id` (heading anchor), `text` (plain text), `level` (2-6), and `children` (nested headings). TOC extraction is controlled by `content.markdown.toc` (default: `true`). Heading anchor IDs are controlled separately by `goldmark.autoHeadingID` -- disabling TOC does not disable heading IDs (see [Content](/content/)).
