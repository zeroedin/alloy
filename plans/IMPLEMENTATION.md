...
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins. Seven categories:
  ...
  - **`onFormatRendered`** (issue #1102): fires once per non-HTML format body after template rendering. The developer must:
    ...
    7. **Test coverage**: 12 spec tests in `internal/pipeline/hooks_test.go` covering: onPageRendered still fires for HTML (default), onPageRendered skips JSON-only pages, onPageRendered fires HTML on multi-format pages, onFormatRendered payload shape validation, content mutation applied, nil return preserves original, return without content key preserves original, does not fire for HTML-only pages, single JSON page fires onFormatRendered, multiple formats fire once each, read-only fields ignored, E2E dual-hook test, non-string content preserves original.
