...
- **HTML front matter detection (issue #337)**: For `.html` files matching `content.formats`, three-way classification based on file content:
  ...
  Detection: read first bytes, check for front matter delimiters first. If no delimiters, check for `<!DOCTYPE` or `<html` (case-insensitive, may have leading whitespace). This happens in discovery, not in `BuildPage`. `.md` files always require front matter — `BuildPage` errors for markdown without delimiters.
