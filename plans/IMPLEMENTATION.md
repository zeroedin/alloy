...
- **HTML front matter detection (issue #337)**: For `.html` files matching `content.formats`, check for front matter delimiters before calling `BuildPage`. Read the first few bytes — if the file starts with `---`, `+++`, or `{`, process as content. If not, add to passthroughs instead. This check happens in `DiscoverWithPassthrough` (or `DiscoverWithFormats`), not in `BuildPage`. `.md` files always go to `BuildPage` — no front matter check bypass.
...
- **Plugin site data access (issue #317)**: Add `SetSiteData(data map[string]interface{})` to `QuickJSRuntime`. JSON-serializes `data` and sets it as `alloy.data` in the JS context via `alloy.data = JSON.parse(__siteDataJSON)`. Called from `Build()` after `loadSiteData(cfg)` completes (step 7) but before content rendering (step 11). The pipeline calls `rt.SetSiteData(siteData)` for each runtime in the registry. For Node (Tier 3), send site data as a `{type: "siteData", payload: data}` message via the bridge. `alloy.data` is read-only — changes in JS do not propagate back to Go.
