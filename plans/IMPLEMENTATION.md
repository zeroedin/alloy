...
- **Plugin site data access (issue #317, wiring #339)**: Add `SetSiteData(data map[string]interface{})` to `QuickJSRuntime`. JSON-serializes `data` and sets it as `alloy.data` in the JS context via `alloy.data = JSON.parse(__siteDataJSON)`. **Pipeline wiring (issue #339)**: After `loadSiteData(cfg)` and external source fetching complete, call `rt.SetSiteData(siteData)` for each runtime in `registry.Runtimes()`. This must happen before content rendering (step 11). Without this call, `alloy.data` is `undefined` in plugins. For Node (Tier 3), send site data as a `{type: "siteData", payload: data}` message via the bridge. `alloy.data` is read-only — changes in JS do not propagate back to Go.
