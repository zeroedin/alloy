...
- `ResolveLayout`: Lookup chain per spec (front matter > section name > filename > default). Handle `layout: false`. **Bare-extension fallback (issue #827)**: When engine is `"liquid"`, each automatic candidate (post, section, filename, default) must try `<name>.liquid` first, then `<name>.html`. Explicit front matter `layout:` candidates try only `.liquid` — no bare-extension fallback for deliberate names. Implementation: for each auto candidate, append both `name+".liquid"` and `name+".html"` to the candidates list (`.liquid` first for priority). For front matter candidates, append only `name+".liquid"`. The Go engine is unchanged — single extension, no fallback.
- `ResolveLayoutWithCascade(page, layoutsDir, engine, permalinkCfg, cascadeData)`: Same lookup chain as `ResolveLayout`, but also considers `_data.yaml` cascade data for layout resolution. Front matter takes priority over cascade data. **Explicit cascade layout names do not get bare-extension fallback** — same rule as front matter (issue #827).
- `ResolveLayoutForFormat`: **Bare-extension fallback (issue #827)**: When engine is `"liquid"`, each format layout candidate tries `<name>.<format>.liquid` first, then `<name>.<format>` (bare extension). Example: `single.json.liquid` → `single.json`.
