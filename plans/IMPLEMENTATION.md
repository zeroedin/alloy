...
- **Hook payload per-page (issue #182)**: `onContentTransformed` and `onPageRendered` must fire once per page with the rendered HTML string as payload — not once with the entire `pages` slice. The pipeline iterates pages and calls `hooks.RunWithTimeout(plugin.OnContentTransformed, page.RenderedContent)` for each page. The returned string replaces the page's content. This makes the payload usable by JS plugins that expect `function(html) { return html.replace(...); }`.
...
- **WASM pipeline bridging (issue #189)**: `Registry.Runtimes()` returns `[]*QuickJSRuntime` only — WASM runtimes are not retained or bridged. The pipeline filter/shortcode bridging loop only iterates QuickJS runtimes. Fix: define a shared `PluginRuntime` interface with canonical signatures matching the pipeline APIs:
  ```go
  ...
  ```
  Then `Runtimes()` returns `[]PluginRuntime` and the bridging loop in `build.go` works for both QuickJS and WASM runtimes without code duplication.
