...
- **File-derived virtual page dependency tracking (issue #1058)**: Optimizes the #970 naive approach (re-render all virtual pages on every incremental rebuild) by making virtual page re-rendering selective based on declared file dependencies. The developer must implement:
  ...
  2. **virtualPageFromMap** (`internal/pipeline/hooks.go`): After constructing the page from the plugin-returned map, read the `dependencies` field. Type-assert as `[]interface{}`, iterate entries, type-assert each as `string`, collect into a `[]string`, and assign to `page.Dependencies`. If the `dependencies` key is absent from the map, `page.Dependencies` remains `nil` (safe fallback). If present but empty (`[]interface{}{}`), assign `page.Dependencies = []string{}` (tracked, empty).
  ...
  4. **BuildIncremental selective rendering** (`internal/pipeline/incremental.go`): Replace the #970 blocks that unconditionally add all virtual pages to `renderRelPaths` (the `previousCache.VirtualPagePaths()` pre-population and the `discoveredPaths` loop) with dependency-aware logic. When `previousCache != nil`: build a `changedFileSet` from `changedFiles`; iterate pages not in `discoveredPaths`; if `page.Dependencies == nil` → add to `renderRelPaths` (safe fallback); if `page.Dependencies` is non-nil → add only if any dependency is in `changedFileSet`. When `previousCache == nil`: add all virtual pages to `renderRelPaths` (initial build, same as current behavior).
  ...
  6. **Test coverage**: 7 cache unit tests in `internal/cache/virtual_dep_test.go` covering: TrackVirtualDependency + InvalidatedVirtualPages roundtrip, nil for unknown source, multiple pages sharing a source, multi-dep invalidation, deduplication, Clone preservation, Clone independence. 5 pipeline integration tests in `internal/pipeline/virtual_dep_incremental_test.go` covering: selective rebuild (matching dep rendered, non-matching skipped, untracked rendered, empty-deps skipped), initial build renders all, shared dependency invalidation, cache tracks dependencies across generations, Build()→BuildIncremental() handoff tracks dependencies.
