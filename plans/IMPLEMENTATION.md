...
- **Data reload edge cases (issue #719, #1018)**: The data reload path handles four edge cases: (1) **loadSiteData error preserves stale data (issue #1018)**: when `loadSiteData` returns a non-nil error during incremental rebuild, the existing `ps.SiteData` is preserved — pages re-render with previous values rather than crashing or producing empty output. `loadSiteData` returns errors specifically from the external data files path (`cfg.Data.Files`): either a missing/unreadable external file or a key collision between an external file and a data directory entry. Data directory parsing errors (e.g., malformed JSON) are swallowed inside `loadSiteData` with a warning log, returning `(nil, nil)` instead. Both error mechanisms preserve stale data — the error path via the `if err != nil` branch, and the nil-data path via the `else` branch that checks directory existence. Test coverage: two tests in `incremental_test.go` exercise the error branch — missing external file and external key collision — verifying the build continues and pages render with stale data. (2) **Malformed data file in data directory**: when a data directory file is malformed, `loadSiteData` logs the parsing error internally and returns `(nil, nil)`. The incremental path detects the nil return, checks that the data directory still exists, and preserves stale `ps.SiteData`. Test coverage in `incremental_test.go` (malformed JSON test, issue #719). (3) **Collections-based pagination**: pages with `pagination.data: "collections.posts"` are NOT invalidated by data file changes — only `site.data.*` sources trigger invalidation. (4) **Non-string pagination.data**: the type assertion chain (`pagination.data` → string → `HasPrefix`) silently skips non-string values without panic. (5) **Custom data directory**: the `dataPrefix` check uses `cfg.Structure.Data` (configured value), falling back to `"data"` only when unconfigured.
