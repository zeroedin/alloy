...
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins. Three categories:
  - **Per-page HTML hooks** (`onContentTransformed`, `onPageRendered`): fire once per page with the rendered HTML string as payload. The pipeline iterates pages and calls `hooks.RunWithTimeout(event, page.RenderedContent)` for each page. The returned string replaces the page's content. Current code passes the entire `pages` slice — must change to per-page iteration.
  ...
  - **Per-build hooks** (`onConfig`, `onBeforeValidation`, `onAfterValidation`, `onDataFetched`, `onContentLoaded`, `onDataCascadeReady`): convert Go structs to `map[string]interface{}` before passing to `CallHook`. Deserialize returned map and apply changes back. `onContentLoaded` and `onDataCascadeReady` fire per page despite being "per-build" category — payload is a per-page JSON object.
  - **Read-only hooks** (`onBuildComplete`, `onDevServerStart`, `onFileChanged`): serialize to JSON for observation, return value ignored.
