...
- **Taxonomy collection items must use `ToTemplateMap()` (issue #328)**: `buildCollectionsContext` in `build.go` converts section collection pages via `ToTemplateMap()` (line 1560) but passes taxonomy terms as raw `[]*content.Page` (line 1567). Fix: iterate `tc.Terms` and convert each page slice via `ToTemplateMap()` before adding to the context. Without this, `{{ p.title }}` and `{{ p.url }}` are empty in taxonomy collection loops.
