...
- **Render hooks (issues #273, #310, #311)**:
  ...
  **Codeblock inner escaping (issues #785, #947)**: `markup.inner` for codeblock hooks must apply two escaping steps before passing to the hook template: (1) HTML escaping via `html.EscapeString` — converts `<`, `>`, `&`, `"`, `'` to entities so HTML tags in code display as text, not markup; (2) Liquid delimiter escaping via `escapeLiquidDelimiters` — converts `{{`, `}}`, `{%`, `%}` to HTML entities so the template engine does not execute them. HTML escaping must run first (on `codeBuf.String()`), then Liquid delimiter escaping on the result. Without HTML escaping, `<h2>` inside a fenced code block renders as an actual heading when the hook template outputs `{{ markup.inner }}` (issue #947). Without Liquid escaping, `{% for %}` examples execute as template code (issue #785). Both functions are called in `renderFencedCodeBlock` in `render_hooks.go`. The `escapeLiquidDelimiters` function remains unchanged; add `html.EscapeString` as a separate call before it.

  **Render hook field escaping (issue #952)**: All render hook context fields carrying raw AST values must be wrapped in `html.EscapeString()` before assignment to the `markup` context map. Fields to escape: `language` in `renderFencedCodeBlock`; `destination`, `text` (from `extractText`), `title` in `renderLink`; `src`, `alt` (from `extractText`), `title` in `renderImage`; `text` (from `extractText`) in `renderHeading`. Fields that must NOT be escaped: `inner` in heading/blockquote/table hooks (already rendered HTML from `renderChildrenToHTML`), `id` (safe — output of `slugifyHeading`), `level` (integer), `is_external` (boolean). Each field gets a simple `html.EscapeString(value)` wrapper at the assignment site in its respective render function.
