...
- `BuildIncremental(cfg, contentMap, previousCache, changedFiles)`: Serve-mode incremental rebuild. Accepts a previous `*cache.Cache` and list of changed file paths. Discovers all pages but only renders those that changed (via `cache.ShouldSkipFile`) or were invalidated (via `cache.InvalidatedPages` for layout changes). Returns `BuildResult` with `PagesSkipped` count. When `previousCache` is nil, renders all pages (equivalent to full build).
