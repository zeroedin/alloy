...
- **`BuildPhase1` removal (issue #1249)**: delete `BuildPhase1` from `internal/pipeline/ssr.go`. It is a test-only reimplementation of Phase 1 that violated the Markdown-only Goldmark invariant (PLAN.md §Phase 1) by running Goldmark on every discovered page — `.html` included — and never rendered templates or applied layouts. SSR Phase 1 is `renderPages`; `Build()` and `BuildIncremental()` already collect its output into the intermediate-HTML map that `BuildPhase2` consumes.
  - Delete the function and its doc comment, then drop the one import it orphans: `internal/content`. `time` and `config` are still used by the rest of the file — verified, not assumed. Keep `BuildPhase2` and its helpers untouched: `BuildPhase2` is production code, called from `build.go` and `incremental.go`. Deleting the wrong half of the pair is the live risk in this change.
  ...
  - The whole change is a 37-line deletion plus that import. It has been performed and validated against this branch: `go build ./...` succeeds and `internal/pipeline`, `internal/content`, and `internal/ssr` all pass.
  - **No changeset.** The removal is internal with no production caller and no user-visible behavior change, which `.changeset/README.md` puts outside the "user-facing changes" rule.
