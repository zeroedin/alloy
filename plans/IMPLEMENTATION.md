...
- **Plugin dependency tracking: addDependencies + invalidateByDependency (issue #1100)**: Enables targeted incremental rebuilds for content pages whose output depends on external files (SSR components, Sass partials, translation files). The developer must implement:
  ...
  5. **ParseFileChangedResult** (`internal/plugin/file_changed.go`): Create a `FileChangedResult` struct with fields `InvalidateByDependency []string`, `Restart bool`, and `Warnings []string`. Create `ParseFileChangedResult(result interface{}) *FileChangedResult` that extracts structured data from the `onFileChanged` hook's return value. Type-assert to `map[string]interface{}` (non-map → nil). Extract `invalidateByDependency` as `[]interface{}` → filter to strings (non-string entries produce a warning, not an error). Extract `restart` as `bool` (default false). Return nil if neither recognized field is present (forward-compatible no-op). Unknown keys are ignored.
