...
- **Typed outbound payload structs (issue #529)**: The outbound serialization path (Go → Plugin) must use typed structs with json tags instead of `map[string]interface{}`. This separates the plugin bridge serialization path from the template rendering path (which uses `map[string]interface{}` for liquidgo). Typed structs allow `sonic.ConfigStd` to JIT-compile optimized encoders for fixed fields — only `FrontMatter` and `SiteData` (user-defined keys) require reflection. The inbound path (Plugin → Go) remains `map[string]interface{}` since plugin return shapes are dynamic. Structs are defined in `internal/plugin/payload.go`:
  ...
  - **Targeted outbound extraction (issue #1185)**: Replace the generic `extractHookResult` (iterates all own properties via `GetOwnPropertyNames` + `JSONStringify` per non-primitive property) with three type-aware extractors. Each reads only the fields the pipeline consumes from the return value:
    ...
    **Identity return tests updated**: The two identity-return spec tests now assert that read-only fields (url, path, frontMatter, format) are NOT present in the result — the targeted extractor drops them because the pipeline never reads them back.
  ...
  - **Hook chain `map[string]interface{}` fast path (issue #1185)**: When hooks chain (first hook's result feeds second hook), the payload is `map[string]interface{}` not a typed struct. Add a `case map[string]interface{}:` branch to the `CallHook` type switch, guarded by `html` or `content` key presence. For page-like maps: build a JS object directly via `NewObject()`/`SetPropertyStr()` — same approach as the typed struct cases. For non-page maps (no `html`/`content` key, e.g., `onBuildComplete`): fall through to the `default` JSON serialization path. Also add the same guard to `EncodeMessage` in `node.go` for the Node IPC split-body path.
    **Test coverage**: 2 spec tests — chain receives html from first hook's map result, non-page map falls through to JSON path.
  - **`BatchCallHook` on `QuickJSRuntime` (issue #1185)**: New method `func (r *QuickJSRuntime) BatchCallHook(name string, payloads []interface{}, onProgress func(int)) ([]interface{}, error)`. Loops synchronously through payloads calling `CallHook` internally, calling `onProgress(i+1)` after each item. No per-item goroutine/channel/context overhead. Registered via the existing `registerRuntime` batch detection (type assertion for `BatchCallHook` method in `registry.go`).
    **Test coverage**: 1 spec test — results match sequential `CallHook` calls for 3 payloads.
