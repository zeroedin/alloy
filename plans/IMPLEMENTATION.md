...
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins. Four categories:
  ...
  - **Per-page JSON hooks** (`onContentLoaded`, `onDataCascadeReady`): `onContentLoaded` fires once with the full pages array (`[]interface{}` of page maps). Return value is applied back: existing pages at indices `0..len(pages)-1` have their `frontMatter` merged. Pages at indices `>= len(pages)` are **virtual pages** (#518) — construct a `*content.Page` from the returned map: `path` → `RelPath`, `url` → `URL`, `content` → `RenderedBody`, `frontMatter` → `FrontMatter` (including `layout`). Validate: `path` or `url` required (error if both missing). Check for URL collisions with existing pages (error on collision). Virtual pages are appended to the `pages` slice and go through the remaining pipeline (layout resolution, template rendering, output writing). `layout: false` in frontMatter skips layout wrapping. `onDataCascadeReady` fires once per page with `map[string]interface{}{"path", "data"}` — no virtual page support.
