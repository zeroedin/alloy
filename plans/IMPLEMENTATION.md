...
- **Hook priority (issue #464)**: `alloy.hook(name, fn, { priority: N })` accepts an optional third argument with a `priority` field. Default priority is 50. Lower priority runs first. Changes needed:
  ...
  - **HookRegistry**: `Register(event, fn, priority int)` stores hooks with priority. `hooks` field changes from `map[HookName][]HookFunc` to `map[HookName][]priorityHook` where `priorityHook` has `fn HookFunc` and `priority int`. `Run`/`RunWithTimeout` sort by priority (stable sort — same priority preserves registration order, which is alphabetical filename order).
