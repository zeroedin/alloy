...
- **Feed template pipeline integration (issue #822)**: After rendering pages (step 15) and before sitemap (step 19), call `ResolveFeedTemplates(layoutsDir)` to discover feed templates. For each discovered feed, render it through the real template engine with the same context available to page templates (`site.*`, `collections.*`, `taxonomies.*`). Write the rendered feed to `outputDir + feedTemplate.OutputPath`. Section feeds receive the section's collection. Per-term taxonomy feed expansion (PLAN.md §4: `layouts/taxonomies/tags/feed.xml` → `/tags/:slug/feed.xml` rendered per term) is deferred to a follow-up issue. `RenderFeedTemplate` must be replaced or refactored to use the real template engine — the current string-substitution implementation does not support loops, filters, or any template logic.
