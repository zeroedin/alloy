...
- **Auto heading IDs + heading attributes (issue #274)**: Enable `parser.WithAutoHeadingID()` and `parser.WithAttribute()` in goldmark parser options. These are goldmark core options, not extensions. Auto heading IDs generate slugified `id` attributes for all headings. Heading attributes let authors override with `{#custom-id .class}`. Both are prerequisites for TOC.
- **TOC extraction (issue #274)**: Add a `TOC` field to `MarkdownOptions` (bool, default true from `cfg.Content.TOC`). When true, `RenderMarkdown` walks the goldmark AST after parsing (before HTML rendering) and collects heading nodes (h2-h6, excluding h1) into a nested `[]TOCEntry` structure. `TOCEntry` has `ID`, `Text`, `Level`, `Children`. Returns the TOC alongside the rendered HTML — change return to `([]byte, []TOCEntry, error)` or add a `TOCResult` wrapper. The pipeline stores the result on `page.TOC` so templates can access `page.toc`. The `onContentTransformed` hook receives the page with `TOC` populated — plugins can mutate it.
