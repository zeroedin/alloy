...
- **Go template block shortcode preprocessor (issue #1002)**: Create `ProcessBlockShortcodes(content []byte, callShortcode func(name string, args []string, innerContent string) (string, error)) ([]byte, error)` in `internal/template/`. This function scans post-Goldmark HTML for `{{% tag "args" %}}...{{% /tag %}}` pairs and replaces each with the shortcode callback's output. Implementation details:
  - **Pattern matching**: Use a regex or scanner to find `{{% ... %}}` delimiters. Opening tags match `{{% tagname "arg1" "arg2" %}}` — extract the tag name and quoted arguments. Closing tags match `{{% /tagname %}}` — extract the tag name for pairing.
  - **Nesting**: Resolve from innermost to outermost. For same-name nesting, use depth tracking: increment on opening `{{% tag %}}`, decrement on `{{% /tag %}}`, only close at depth zero.
  - **Error handling**: Return an error (with the tag name) if an opening tag has no matching closing tag, or if closing tag names don't match. Propagate callback errors as build errors.
