...
- **Go template block shortcode preprocessor (issue #1002)**: Create `ProcessBlockShortcodes(content []byte, callShortcode func(name string, args []string, innerContent string) (string, error)) ([]byte, error)` in `internal/template/`. This function scans post-Goldmark HTML for `{{% tag "args" %}}...{{% /tag %}}` pairs and replaces each with the shortcode callback's output. Implementation details:
  - **Pattern matching**: Use a stack-based scanner to find `{{% ... %}}` delimiters. Opening tags match `{{% tagname "arg1" "arg2" %}}` — extract the tag name and quoted arguments. Closing tags match `{{% /tagname %}}` — extract the tag name for pairing. The scanner must skip content inside `<pre>` and `<code>` elements — `{{% %}}` delimiters in code blocks are literal text, not shortcode invocations. The existing `escapeTemplateTagsInCode` step handles `{{ }}` and `{% %}` but not `{{% %}}`, so the preprocessor itself must be code-block-aware.
  - **Nesting**: Use a stack: push each opening `{{% tag %}}` as a frame (name + position). Each closing `{{% /tag %}}` must match the top of the stack — if names don't match, return an error. Pop the matched frame and process the innermost pair (call the shortcode callback, replace the block with output). Continue until the stack is empty. This naturally resolves innermost-to-outermost and rejects mixed mismatches (e.g., `{{% outer %}}{{% inner %}}{{% /outer %}}` errors because `/outer` doesn't match stack top `inner`).
  - **Error handling**: Return an error (with the tag name) if an opening tag has no matching closing tag (non-empty stack at end of input), or if a closing tag doesn't match the stack top. Propagate callback errors as build errors.
  ...
  - **Pipeline integration**: In `renderPages()` (render.go), when the configured engine is `"gotemplate"`, call `ProcessBlockShortcodes` on the post-Goldmark HTML before passing it to `rc.Engine.Parse()`. The callback routes through the plugin registry's `CallShortcode`. This step runs after `escapeTemplateTagsInCode` and before `hasTemplateSyntax`/template rendering.
