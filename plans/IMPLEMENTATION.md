...
- **Unified plugin bridge (issues #189, #237)**: All plugin tiers (Tier 2 QuickJS, Tier 2 WASM, Tier 3 Node) must implement the same `Runtime` interface. `Registry.Runtimes()` returns `[]Runtime` and the bridging loop in `Registry.LoadPlugins()` works identically for all tiers — no tier-specific code.
  ...
  **NodeRuntime specifics:**
  ...
  - **Stop() timeout escalation (issue #726)**: When a Node subprocess ignores stdin close and SIGTERM, `Stop()` must escalate to SIGKILL and complete within bounded time. The SIGTERM → SIGKILL fallback path must be exercised by spec tests using `SetBridgeCommand` to inject a SIGTERM-resistant process. Validated by: (1) process is dead after Stop, (2) Stop completes within 15s (expected ~7s: 5s SIGTERM grace + 2s SIGKILL grace).
  - **Windows test coverage (issue #726)**: `node_windows.go` must have dedicated spec tests (`//go:build windows`) validating: process lifecycle state machine, PID file creation/removal with mutex-based locking (no flock), stale PID cleanup, malformed PID file handling, and concurrent multi-bridge PID file writes. These mirror the POSIX specs but use Windows-compatible process checks (no `syscall.Kill(pid, 0)` or `syscall.Getpgid`).
