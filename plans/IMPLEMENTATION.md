...
**page.go**:
- `SetRenderedHTML(s string)` (issue #1189): Stores both `RenderedBody = []byte(s)` and `renderedStr = s` simultaneously. Avoids the string→`[]byte`→string round-trip that `SetRenderedBody([]byte(html))` triggers when the caller has a Go string (hook result application, QuickJS `GetPropertyStr` output). `SetRenderedBody` must still clear `renderedStr = ""` (existing behavior) so code paths that set raw `[]byte` don't serve a stale cached string from a prior `SetRenderedHTML`. The pipeline uses `SetRenderedHTML` in all hook result apply-back sites in `hooks.go` where the source value is a Go string: `fireContentTransformedHooks` (the `html` key from `toGoMap(result)` and the raw string fallback) and `dispatchPostRenderHooks` (the `html` key from batch results). Non-hook paths (`render.go`, `taxonomy.go`, `build.go`, `incremental.go`) that receive actual `[]byte` (from template rendering) continue to use `SetRenderedBody`. Test coverage: 6 specs in `internal/content/page_set_html_test.go` — HTML() correctness, RenderedBody correctness, overwrites previous state, empty string, restore after ReleaseRenderedBody, and SetRenderedBody-after-SetRenderedHTML cache invalidation.
...
- **Typed outbound payload structs (issue #529)**: The outbound serialization path (Go → Plugin) must use typed structs with json tags instead of `map[string]interface{}`. This separates the plugin bridge serialization path from the template rendering path (which uses `map[string]interface{}` for liquidgo). Typed structs allow `sonic.ConfigStd` to JIT-compile optimized encoders for fixed fields — only `FrontMatter` and `SiteData` (user-defined keys) require reflection. The inbound path (Plugin → Go) remains `map[string]interface{}` since plugin return shapes are dynamic. Structs are defined in `internal/plugin/payload.go`:
  ...
  - **Hook chain `map[string]interface{}` fast path (issue #1185)**: When hooks chain (first hook's result feeds second hook), the payload is `map[string]interface{}` not a typed struct. Add a `case map[string]interface{}:` branch to the `CallHook` type switch, guarded by `html` or `content` key presence. For page-like maps: build a JS object directly via `NewObject()`/`SetPropertyStr()` — same approach as the typed struct cases. For non-page maps (no `html`/`content` key, e.g., `onBuildComplete`): fall through to the `default` JSON serialization path. Also add the same guard to `EncodeMessage` in `node.go` for the Node IPC split-body path.
    **Test coverage**: 3 spec tests — chain receives html from first hook's map result, chain receives content from first hook's map result (onFormatRendered scenario — verifies `content` key guard, not just `html`), non-page map falls through to JSON path.
