...
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins. Five categories:
  ...
  - **Content hooks** (`onContentLoaded`, `onDataCascadeReady`): `onContentLoaded` fires once with the full pages array (`[]interface{}` of page maps with keys `path`, `url`, `frontMatter`, `content`, `html`). Return value is applied back: existing pages at indices `0..len(pages)-1` have their `frontMatter`, `html`, `toc`, and other mutable fields merged. **Virtual page injection is not supported** — if the returned array length exceeds the input length, produce a validation error. Virtual page injection belongs exclusively in `onPagesReady` (#525). This also resolves #521 (virtual pages appended to wrong language batch) — since `onContentLoaded` no longer accepts virtual pages, the batch routing problem is eliminated. `onDataCascadeReady` must serialize its payload the same way `onContentLoaded` does — call `serializePagesForHook(pages)` before the hook, apply returned cascade data back. Per-page payload: `map[string]interface{}{"path": page.RelPath, "data": cascadeData}`. No virtual page support.
