...
- **`BuildPhase1` removal (issue #1249)**: delete `BuildPhase1` from `internal/pipeline/ssr.go`. It is a test-only reimplementation of Phase 1 that violated the Markdown-only Goldmark invariant (PLAN.md §Phase 1) by running Goldmark on every discovered page — `.html` included — and never rendered templates or applied layouts. SSR Phase 1 is `renderPages`; `Build()` and `BuildIncremental()` already collect its output into the intermediate-HTML map that `BuildPhase2` consumes.
  - Drop the now-unused imports from `ssr.go` (`content`, `time`, and `config` if nothing else in the file uses them) and keep `BuildPhase2` and its helpers untouched — `BuildPhase2` is production code, called from `build.go` and `incremental.go`.
  ...
  - **Verify**: `grep -rn "BuildPhase1" --include='*.go' .` returns nothing, and `go build ./... && go test ./internal/pipeline/ ./internal/content/` passes. A stale prose reference in `internal/content/markdown_test.go` (a comment naming `BuildPhase1` as a TOC-discarding caller) is already corrected on this branch.
