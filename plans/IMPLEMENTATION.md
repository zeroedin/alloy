...
- **Render hooks (issues #273, #310, #311)**:
  ...
  **Goldmark integration**: For each hook type in `opts.Hooks`, register a custom `renderer.NodeRenderer` that builds the `markup.*` context from the AST node and calls `opts.HookRenderer(templateSrc, ctx)`. Context: `inner` from rendered children, `language` from fenced code block, `level`/`id` (custom `{#id}` attribute if present, otherwise auto-slug)/`inner` (rendered HTML)/`text` (plain text)/`attributes` (goldmark-parsed `{.class #id}` map) from heading, `src`/`alt`/`title` from image, `destination`/`text`/`title`/`is_external` from link. Render hooks receive only `markup.*` — no `page.*` or `site.*` context. Heading `attributes` are extracted via `node.Attributes()` and converted to `map[string]interface{}` (issue #824). Attribute support for other block elements requires custom goldmark parser extensions (issue #892).
  
  **Codeblock inner escaping (issue #785)**: `markup.inner` for codeblock hooks must have Liquid delimiters (`{{`, `}}`, `{%`, `%}`) entity-escaped before passing to the hook template. Without this, Liquid syntax in code fences (e.g., `{% for %}` examples) is executed by the template engine instead of displayed literally. The escaping uses HTML entities (`&#123;&#123;` etc.) — same approach as `escapeTemplateTagsInCode` in `render.go` but applied at the source (render hook context) rather than post-hoc via regex on `<code>` elements. This ensures code content is safe regardless of what HTML structure the hook template produces. The unexported `escapeLiquidDelimiters(s string) string` in `render_hooks.go` handles this, called on `codeBuf.String()` at line 84.
...
- **Includes via `include` function (issues #823, #883)**: Register an `"include"` FuncMap function that resolves and renders partial templates from the layouts directory. Named `include` for parity with Liquid's `{% include %}`. Implementation:
  ...
  - Nesting depth: engine-scoped `atomic.Int32` (`goEngine.depth`). Increment on entry, decrement on exit (via defer). Build error at depth > 100 with message containing "too deep" or "nesting" (matches Ruby/Go Liquid's `StackLevelError`). The counter is thread-safe (atomic), but concurrent renders on the same engine instance share it — under heavy concurrent load this could produce false "nesting too deep" errors. Currently latent: the pipeline renders sequentially. Concurrent rendering safety is spec-tested (issue #884).
  - **`dict` error on odd args (issue #884)**: `dict` returns `(nil, fmt.Errorf("dict requires even number of arguments, got %d", len(pairs)))` when given an odd argument count. This is a render-time error (parse succeeds because `dict` is variadic).
