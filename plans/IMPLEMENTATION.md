...
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins. Seven categories:
  ...
  - **onConfig merge-back (issue #973)**: The `Build()` call site currently discards the `onConfig` return value (`_, err := hooks.RunWithTimeout(plugin.OnConfig, cfg)`). The developer must:
    ...
    2. Create a function `applyOnConfigResult(cfg *config.Config, result interface{}) error` in `internal/pipeline/build.go` (or a new `config_apply.go` in the same package). This function:
       ...
       - **Passthrough path validation (issues #1031, #1034)**: Extend path-containment validation to passthrough `from` and `to` fields. During the passthrough deserialization loop in `applyOnConfigResult`, validate each `from` value with `validateOnConfigPath(fmt.Sprintf("passthrough[%d].from", i), from)` — same rules as `build.output` (reject absolute, traversal, and `.`). For `to`, apply `validateOnConfigPath` only when the value is non-empty and not `"."` — `to: "."` and `to: ""` are valid (they mean "root of the output directory"), unlike `build.output` where `"."` targets the project root for `CleanOutputDir` deletion. When `to` is non-empty and non-`"."`, validate with `validateOnConfigPath(fmt.Sprintf("passthrough[%d].to", i), to)`. All passthrough path validations must pass before the passthrough slice is written to `cfg.Passthrough` — collect validated mappings first, then apply only after the full loop succeeds. Passthrough validation must also complete before any structure/build fields are applied, to maintain the existing atomicity guarantee (no partial config mutation on failure). Test coverage: 14 spec tests in `internal/pipeline/config_apply_test.go`.
