...
- **Plugin filter/shortcode call error propagation (issue #1224)**: `registerPluginExtensions` in `state.go` wraps `CallFilter` and `CallShortcode` in closures for the template engine. When the call returns an error, the closure panics with a `PluginCallError` (`internal/template/liquid.go`) instead of silently returning the original input (filters) or empty string (shortcodes). `PluginCallError` implements `liquid.LiquidError` so liquidgo's internal recover preserves the error message (non-`LiquidError` panics are replaced with a generic "internal" string by liquidgo). For Go templates, `html/template.Execute`'s `errRecover` catches any panic satisfying the `error` interface and returns it as the Execute error. Both engines already wrap render errors with the template name (which is the page's `RelPath`), so the error message includes both the filter/shortcode name and the page path without additional changes to the render pipeline. For shortcodes in Go template mode, `processGoTemplateBlockShortcodes` already propagates `CallShortcode` errors correctly — the gap was only in the `engine.AddTag` wrapper in `registerPluginExtensions`. Test coverage: 6 specs + 1 regression guard in `internal/pipeline/build_test.go` under "Filter/shortcode call error propagation (issue #1224)".
