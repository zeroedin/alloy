...
- **Plugin filter/shortcode call error propagation (issue #1224)**: The current `registerPluginExtensions` in `state.go` wraps `CallFilter` and `CallShortcode` in closures that swallow errors — returning the original input (filters) or empty string (shortcodes). This must change: when `CallFilter` or `CallShortcode` returns an error, that error must propagate as a template render error that fails the page. The challenge is that `FilterFunc` returns `interface{}` (no error channel) and liquidgo's filter dispatch uses reflection on bridge methods that also return `interface{}`. The developer must add an error propagation mechanism — either an error accumulator on the template engine (checked after `Render()` returns), or a change to `FilterFunc` to return `(interface{}, error)` with corresponding changes to both engines. The same mechanism must work for both Liquid and Go template engines, and for shortcode calls. For shortcodes in Go template mode, `processGoTemplateBlockShortcodes` already propagates `CallShortcode` errors correctly — the gap is only in the `engine.AddTag` wrapper in `registerPluginExtensions`. Test coverage: 6 specs in `internal/pipeline/build_test.go` under "Filter/shortcode call error propagation (issue #1224)".
