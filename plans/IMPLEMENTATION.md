...
- **Native qjs value conversion (issue #465)**: `QuickJSRuntime.CallHook` (wasm.go:306) currently uses JSON as the Go↔QuickJS transport: `json.Marshal` → write bytes → `JSON.parse` → JS work → `JSON.stringify` → read bytes → `json.Unmarshal`. Replace with the native `qjs` API: `qjs.ToJsValue(ctx, goValue)` for Go→JS (builds JS objects via `SetPropertyStr`, no JSON), and read result properties back via `Value.GetPropertyStr()` / `jsValueToGo()` for JS→Go. The `qjs` library already has `GoMapToJs` (handles `map[string]interface{}` recursively), `GoSliceToJs`, and all primitive converters. The JS-side `hook-call.js` eval must also change: remove `JSON.stringify` from the result — return the JS object directly and let Go read it via `GetPropertyStr`. Also applies to `CallFilter` (line 215, args serialization), `SetSiteData` (line 120, site data injection), and WASM `CallExport` string args (line 552). **Note**: `*ordered.Map` values in the payload must be pre-converted to `map[string]interface{}` (#463) before passing to `ToJsValue` — `ordered.Map` has unexported fields that reflection cannot access.
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins (or converted to native qjs values per #465). Four categories:
