...
- **WASM pipeline bridging (issue #189)**: `Registry.Runtimes()` returns `[]*QuickJSRuntime` only — WASM runtimes are not retained or bridged. The pipeline filter/shortcode bridging loop only iterates QuickJS runtimes. Fix: either add `WASMRuntimes() []*WASMRuntime` to Registry, or define a shared `PluginRuntime` interface with `RegisteredFilters() []string`, `CallFilter(name, input, args) (interface{}, error)`, `RegisteredShortcodes() []string`, `CallShortcode(name, args, content) (string, error)` — then `Runtimes()` returns `[]PluginRuntime` and the bridging loop works for both.
