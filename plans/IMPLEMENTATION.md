...
- **Render hooks (issue #273)**: `RenderMarkdown` must accept an optional map of render hook templates (keyed by type: `"codeblock"`, `"link"`, `"image"`, `"heading"`, `"blockquote"`, `"table"`). For each provided hook, register a custom goldmark `renderer.NodeRenderer` that calls the Liquid engine with the node's context instead of emitting default HTML. The pipeline scans `layouts/_markup/` at startup for `render-{type}.liquid` files and passes them to `RenderMarkdown`. Language-specific code block hooks (`render-codeblock-{language}.liquid`) are checked first, falling back to the generic `render-codeblock.liquid`. Each template is parsed once and reused across all pages. The render hook context (`markup.*`) is built from the goldmark AST node: `inner` from rendered children, `language` from `ast.FencedCodeBlock.Language()`, `level` from `ast.Heading.Level`, etc. `page.*` and `site.*` are passed through from the render call's context.
