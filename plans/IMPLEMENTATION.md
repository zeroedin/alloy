...
- **Hook priority (issue #464)**: `alloy.hook(name, fn, { priority: N })` accepts an optional third argument with a `priority` field. Default priority is 50. Lower priority runs first. Changes needed:
  - **JS bridge**: `alloy.hook()` in bridge.js captures the third argument's `priority` field. `__registerHook` callback sends `{name, priority}` instead of just `name`. Default to 50 if omitted.
  - **QuickJS/Node runtime**: `RegisteredHooks()` returns `[]HookRegistration{Name, Priority}` instead of `[]string`. Or add a separate `RegisteredHookPriorities() map[string]int`.
  - **HookRegistry**: Add `RegisterWithPriority(event, fn, priority int)`. `Register(event, fn)` delegates to `RegisterWithPriority(event, fn, 50)`. `hooks` field changes from `map[HookName][]HookFunc` to `map[HookName][]priorityHook` where `priorityHook` has `fn HookFunc` and `priority int`. Use insertion sort at registration time to maintain sorted order — do NOT sort per-call in `Run`/`RunWithTimeout` (those fire per-page, sorting per-call is wasteful). Stable insertion preserves registration order for same-priority hooks.
  - **registry.go**: `loadPluginRuntime` passes priority from `RegisteredHooks()` into `hooks.Register(event, fn, priority)`.
...
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins. Five categories:
  - **`onContentTransformed`** (issue #448): fires once per page with a page object payload: `map[string]interface{}{"html": string(page.RenderedBody), "toc": page.TOC, "path": page.RelPath, "url": page.URL, "frontMatter": page.FrontMatter}`. The returned object is applied back: `html` → `page.RenderedBody`, `toc` → `page.TOC`, `frontMatter` → `page.FrontMatter`. This allows plugins to build TOC for non-markdown pages, modify front matter post-render, or transform HTML. `fireContentTransformedHooks` must serialize the page object, not just the HTML string. **Performance (issue #463)**: Before building the payload, recursively convert any `*ordered.Map` values in `page.FrontMatter` to plain `map[string]interface{}` via `ToGoMap()`. `ordered.Map.MarshalJSON` is expensive (13.7% of CPU in profiling). The conversion is read-only — `page.FrontMatter` is not mutated, only the serialization copy is converted. Templates still see the original `*ordered.Map` values after hooks complete.
  ...
  - **Pre-taxonomy hook** (`onPagesReady`, issue #525): fires once per language batch inside `applyBatchContext()`, after cascade data application (line ~2238) but before `BuildTaxonomies()` (line ~2245). Payload: `map[string]interface{}{"pages": serializePagesForPagesReady(pages), "siteData": siteData}` where each page is `map[string]interface{}{"path": page.RelPath, "url": page.URL, "frontMatter": page.FrontMatter, "content": string(page.RawBody)}`. No `html` field — content has not been rendered yet. Return value: same shape. Pages at indices `>= len(originalPages)` are virtual pages — construct `*content.Page` from returned map: `path` → `RelPath`, `url` → `URL`, `frontMatter` → `FrontMatter` (including taxonomy terms like `tags`), `content` → `RawBody`. Validate: `path` and `url` required (error if missing). Check for URL collisions with existing pages (error on collision). Appended virtual pages are added to the `pages` slice **before** `BuildTaxonomies()` and `buildCollectionsContext()` — so they participate in taxonomy collection and appear in `taxonomies.*` in templates. Virtual pages with raw `content` flow through the content rendering pipeline (markdown → HTML). Per-batch firing means each invocation only sees pages from one language batch, avoiding the language routing problem in #521. `serializePagesForPagesReady` is similar to `serializePagesForHook` but omits `html` and includes `content` (raw body).
  - **Content hooks** (`onContentLoaded`, `onDataCascadeReady`): `onContentLoaded` fires once with the full pages array (`[]interface{}` of page maps with keys `path`, `url`, `frontMatter`, `content`, `html`). Return value is applied back: existing pages at indices `0..len(pages)-1` have their `frontMatter` merged. Pages at indices `>= len(pages)` are **virtual pages** (#518) — construct a `*content.Page` from the returned map: `path` → `RelPath`, `url` → `URL`, `html` → `RenderedBody`, `frontMatter` → `FrontMatter` (including `layout`). Validate: `path` and `url` required (error if missing). Check for URL collisions with existing pages (error on collision). Virtual pages are appended to the `pages` slice and go through the remaining pipeline (layout resolution, template rendering, output writing). `layout: false` in frontMatter skips layout wrapping. `onDataCascadeReady` must serialize its payload the same way `onContentLoaded` does — call `serializePagesForHook(pages)` before the hook, apply returned cascade data back. Per-page payload: `map[string]interface{}{"path": page.RelPath, "data": cascadeData}`. No virtual page support.
