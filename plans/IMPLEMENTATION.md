...
- `ResolveLayoutWithCascade(page, layoutsDir, engine, permalinkCfg, cascadeData)`: Same lookup chain as `ResolveLayout`, but also considers `_data.yaml` cascade data for layout resolution. Front matter takes priority over cascade data. **Explicit cascade layout names are deliberate** — same hard-error semantics as front matter: try only `.liquid`, error if missing (issue #827).
