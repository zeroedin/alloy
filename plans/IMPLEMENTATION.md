...
- **Data files preserve author key order (issue #1262)**: Contract in PLAN.md → "Data files preserve author key order (issue #1262)". `LoadFileAny` must return `*ordered.Map` for YAML and TOML objects, as it already does for JSON. This supersedes the "The `LoadFile` return type stays `map[string]interface{}` for YAML/TOML" note above, for `LoadFileAny` only — `LoadFile` keeps flattening through `ToGoMap` and keeps its `map[string]interface{}` signature.
  ...
  **Reapply the four semantics the node walk bypasses.** `yaml.Unmarshal` is still what parses the document — `decodeYAMLOrdered` calls it. What unmarshalling into a `yaml.Node` bypasses is the decoder's *value path*: `yaml.v3` sees the destination is a `Node`, copies it and returns (`decode.go`, immediately before its `switch n.Kind`), so none of the checks that path performs ever execute. Measured on identical bytes, a duplicate key and a self-referential alias both error into an `interface{}` and both return `nil` into a `yaml.Node`. Anything that decoder did for you is therefore now yours to do. Two of these keep the output correct; the other two keep the process alive, and were found only after the first implementation shipped (issue #1271).

  - **Duplicate mapping keys.** `yaml.Unmarshal` errors; the node walk will not. Detect a repeated key within a mapping and return a parse error naming both the repeat and the first definition — the second line alone does not tell the author where to look. `ordered.Map.Set` will not help, because it silently overwrites and the file would be accepted. **This covers a repeated `<<` as well:** `yaml.Node` keeps two merge keys as separate `Content` pairs and the plain decoder rejects the repeat like any other duplicate, so duplicate tracking has to see merge keys *before* the merge branch skips them. Track every key including `<<` in one set, and keep a second set of non-merge keys for local-key precedence. Guarded by "still rejects duplicate mapping keys" and "rejects a duplicate merge key".
  - **Merge keys and aliases.** Expand `<<` into the parent mapping, with local keys winning over merged ones and, among several merge sources, the earlier one winning. Follow `AliasNode` for both scalars and collections. A plain `Content` loop produces a literal `"<<"` key and drops the merged-in keys entirely. Guarded by "still resolves merge keys and aliases", which asserts through `LoadFile` so it holds before and after the change.
  - **Alias cycles.** `a: &a [*a]` makes the walk re-enter the node it is already expanding. This is not a slow build or a recoverable panic — it is a **fatal stack overflow** that kills the process, names no file, and cannot be caught by any caller. Keep a set of the alias nodes currently being expanded and reject a node reached while already in it, with the plain decoder's wording (`anchor "a" value contains itself`). The set must be threaded through the merge-key path too: `<<: *self` reaches the same recursion by a different route. Guarded by "rejects a self-referential alias instead of overflowing the stack".
  - **An alias expansion budget.** Repeated aliases expand a compact document non-linearly; `yaml.v3` bounds that work against document size and the node walk inherits none of it. Without it the loader **hangs** rather than failing. Mirror `yaml.v3` rather than inventing a limit — the same `decodeCount` / `aliasCount` / `aliasDepth` counters, the same `aliasCount > 100 && decodeCount > 1000` trigger, and the same ratio curve across 400,000–4,000,000 — so a document this walk accepts is one the plain decode accepted. Guarded by "rejects a document whose aliases expand without bound", whose fixture is asserted to be rejected by the plain decoder too.

  **The two alias guards are not redundant, and the walk cannot be free functions.** Both need per-document state, so the receiver has to be threaded through the value, mapping and merge-source paths. Verified by removing each in turn: with the cycle guard gone the budget catches the recursive fixture instead, so the *budget* is what prevents the crash while the *cycle guard* is what produces the accurate message; with the budget gone the suite hangs. A spec guarding only one leaves the other free to regress.
  ...
  **Record implicit parents as you walk past them.** `MetaData.Keys()` returns a dotted key as its full path — `zebra.value = 1` yields `[]string{"zebra", "value"}` — but emits no separate terminal entry for the implicit parent `zebra`. The parent is therefore only ever an intermediate path segment, never terminal, so unless you append it to its level's key list while traversing the path it never gets a position and falls into the sorted remainder — a file reading `zebra, apple` renders `apple, zebra`, which is the exact defect this issue exists to remove. This is a separate trap from the array-of-tables ambiguity below, at the same call site, and the original fixtures missed it because `[table]` headers *are* reported as terminal keys (issue #1271). Guarded by "preserves TOML key order for dotted keys with no table header", which also asserts the implicit parent still carries its children — recording the position without descending passes a root-only assertion.
...
### 2B: `internal/content` — 76 tests
**Files**: `frontmatter.go`, `discovery.go`, `markdown.go`, `lifecycle.go`, `page.go`

**page.go**:
- `SetRenderedHTML(s string)` (issues #1185, #1189): Stores both `RenderedBody = []byte(s)` and `renderedStr = s` simultaneously. Avoids the string→`[]byte`→string round-trip that `SetRenderedBody([]byte(html))` triggers when the caller has a Go string. `SetRenderedBody` must still clear `renderedStr = ""` (existing behavior) so code paths that set raw `[]byte` don't serve a stale cached string from a prior `SetRenderedHTML`. See §4A pipeline performance section for call site integration details.
  **Test coverage**: 6 specs in `internal/content/page_set_html_test.go` — HTML() correctness, RenderedBody correctness, overwrites previous state, empty string, restore after ReleaseRenderedBody, and SetRenderedBody-after-SetRenderedHTML cache invalidation.
...
- **`BuildPhase1` removal (issue #1249)**: delete `BuildPhase1` from `internal/pipeline/ssr.go`. It is a test-only reimplementation of Phase 1 that violated the Markdown-only Goldmark invariant (PLAN.md §Phase 1) by running Goldmark on every discovered page — `.html` included — and never rendered templates or applied layouts. SSR Phase 1 is `renderPages`; `Build()` and `BuildIncremental()` already collect its output into the intermediate-HTML map that `BuildPhase2` consumes.
  - Delete the function and its doc comment. That orphans the `internal/content` import: all five `content.` references in the file are inside `BuildPhase1`. `time` and `config` stay — `buildPhase2Exec` and `buildPhase2Stream` use them for timeout parsing and the `*config.SSRConfig` parameter.
  - Keep `BuildPhase2` and its helpers untouched: `BuildPhase2` is production code, called from `build.go` and `incremental.go`. Deleting the wrong half of the pair is the live risk in this change.
  - Verified against this branch by trial deletion: the change is 37 lines plus the one import, `go build ./...` succeeds, and `internal/pipeline`, `internal/content`, and `internal/ssr` all pass. There is no hidden coupling to unpick.
  - The two `ssr_test.go` specs that drove `BuildPhase1` ("Phase 1 produces intermediate HTML preserving raw custom element tags" and "Phase 2 receives Phase 1 output as its input") are rewritten in this branch to produce Phase 1 output via `BuildWithContent` and hand it to `BuildPhase2`, so no test references remain to delete.
  - **Verify**: `go build ./... && go test ./...` passes, and `grep -rn "BuildPhase1" --include='*.go' .` returns exactly one match — a historical comment in `internal/pipeline/html_markdown_invariant_test.go` explaining what the guard tests exist to prevent. There must be no *code* references. (A stale prose reference in `internal/content/markdown_test.go` is already corrected on this branch.)
- `BuildPhase2`: Phase separation. Phase 2 operates entirely in memory:
...
- **Copy sources in conflict detection (issue #1238)**: `outputEntries` in `build.go` is built from pages, per-format outputs, aliases, taxonomy pages, and plugin `addOutputs`. Four file-copy sources — `static/`, `assets/`, resolved passthrough, and content-colocated files — never reach it, so collisions between them resolve silently by write order. The spec (PLAN.md §Pre-Build Validation) now lists all eleven sources with their error labels.

  **Ordering problem to solve first.** Conflict detection runs at `build.go:574`, before rendering. The copy sources are enumerated much later, at `build.go:994-1020`, during the copy stage. The walks therefore have to move earlier, or — better — be computed once into a shared structure that both detection and the copy stage consume, so the directory trees are walked once per build rather than twice. Whichever shape is chosen, detection must still run before any file is written; a conflicting build must leave no partial output.

  **The scanned set must equal the copied set.** Passthrough `exclude` patterns and glob `from` resolution decide what is actually copied, so they must be applied while collecting entries. `static.NormalizeExcludePatterns` / `static.MatchExcludeNormalized` / `static.ContainsGlobChars` / `static.GlobRoot` already implement this logic for the copy and watcher paths — reuse them rather than reimplementing, or the collected set will drift from the copied set. `CopyPassthroughWithValidation`'s managed-directory skip is a *source-side* check and must be applied too, or a mapping that is skipped at copy time gets reported as a conflict.

  **`internal/validation/conflicts.go` needs no change.** `DetectConflicts` already groups by exact path and returns every conflict with every source. Only its input set is incomplete, and only the error formatting at `build.go:576` truncates — it reports `conflicts[0]` and `Sources[0]`/`Sources[1]` alone. Per spec, report all conflicts and all sources per conflict.

  **Dev mode.** `DetectConflicts` is currently called only from `Build()`. `alloy dev`/`alloy serve` handle static, asset, and passthrough changes directly in `cmd/dev.go` (~line 180-225) and `cmd/serve.go` (~line 134) by computing a destination and calling `fileutil.CopyFile`; `BuildIncremental` does not copy those sources at all and does not handle content-colocated files either. To satisfy the spec's dev rules, the claim set has to outlive a single `Build()` call — carry it on the pipeline state so the recopy path can check a destination before writing, and skip the write on conflict rather than performing it. Surface the conflict through the existing overlay API (`srv.Overlay().SetErrors([]server.BuildError{...})`, cleared via `ClearErrors()`), which `cmd/dev.go` already uses for build and render errors. A dev conflict must not terminate the server.

  **Testability.** Both `Build()` and the dev recopy path need the same collection logic, and `internal/pipeline`'s tests are an external test package (`package pipeline_test`), so an unexported helper cannot be unit-tested directly. Export the collection entry point — or place it in `internal/validation` alongside the detector — so the claim set can be asserted without driving a full build. End-to-end coverage still goes through `BuildWithContent`, matching the existing conflict tests in `taxonomy_test.go` and `hooks_test.go`.
...
- **Plugin filter/shortcode call error propagation (issue #1224)**: `registerPluginExtensions` in `state.go` wraps `CallFilter` and `CallShortcode` in closures. When a call returns an error, the closure panics with `PluginCallError` (defined in `internal/template/liquid.go`). `PluginCallError` carries the kind (`"filter"` or `"shortcode"`), name, and wrapped error. It implements `liquid.LiquidError` (via `GetError() *liquid.Error`) so liquidgo's internal recover preserves the error message instead of replacing it with a generic `"internal"` string. Go's `html/template.Execute` has an `errRecover` that catches any panic satisfying the `error` interface, so the same type works for both engines without changing `FilterFunc` or `TagFunc` signatures. Both engines' existing render error wrapping adds the page path automatically. For shortcodes in Go template mode, `processGoTemplateBlockShortcodes` already propagates `CallShortcode` errors correctly — the panic mechanism covers only the `engine.AddTag` wrapper in `registerPluginExtensions`. Test coverage: 6 specs in `internal/pipeline/build_test.go` under "Filter/shortcode call error propagation (issue #1224)".
...
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins. Seven categories:
  - **`onContentTransformed`** (issue #448): fires once per page with a typed `HookTransformPayload` struct: `HookTransformPayload{HTML: string(page.RenderedBody), TOC: convertTOCEntries(page.TOC), Path: page.RelPath, URL: page.URL, FrontMatter: convertOrderedMaps(page.FrontMatter)}` where `convertTOCEntries` copies `[]content.TOCEntry` → `[]plugin.TOCEntry` (shallow field copy, recursive for children). The returned object (inbound as `map[string]interface{}`) is applied back: `html` → `page.RenderedBody`, `toc` → `page.TOC`. **`frontMatter` is read-only context — it is NOT applied back from the return value** (issue #1201, consistent with `onPageRendered` and `onFormatRendered`). The `frontMatter` field is sent to JS for conditional logic only. The developer must remove the `frontMatter` apply-back block from `fireContentTransformedHooks` (the `modified["frontMatter"]` extraction and `page.FrontMatter = returnedFM` assignment). This allows plugins to build TOC for non-markdown pages or transform HTML. `fireContentTransformedHooks` must build a typed `HookTransformPayload`, not a `map[string]interface{}`. **Performance (issue #463)**: Before building the payload, recursively convert any `*ordered.Map` values in `page.FrontMatter` to plain `map[string]interface{}` via `ToGoMap()`. `ordered.Map.MarshalJSON` is expensive (13.7% of CPU in profiling). The conversion is read-only — `page.FrontMatter` is not mutated, only the serialization copy is converted. Templates still see the original `*ordered.Map` values after hooks complete. **Performance (issue #529)**: Use `sonic.ConfigStd` for JSON serialization instead of `encoding/json`. Typed payload structs allow sonic's JIT compiler to generate optimized encoders for the fixed fields (`Path`, `URL`, `HTML`, `TOC`) — only `FrontMatter` (user-defined keys) requires reflection. See §Typed outbound payload structs below.
  - **`onPageRendered`** (issues #1095, #1102): fires once per page **only for pages with HTML output** — i.e., `page.Outputs` is nil/empty (defaults to `["html"]`) or contains `"html"`. Pages with only non-HTML outputs (e.g., `outputs: ["json"]`) are excluded from `onPageRendered` and route through `onFormatRendered` instead. Uses a typed `HookRenderedPayload` struct: `HookRenderedPayload{HTML: page.HTML(), FrontMatter: convertOrderedMaps(page.FrontMatter), URL: page.URL, Path: page.RelPath}`. The `HookRenderedPayload` struct exists in `internal/plugin/payload.go` with JSON tags `html`, `frontMatter`, `url`, `path`. The apply-back block extracts `html` from the returned map via `toGoMap(result)`: if the result is a map with an `"html"` key (string), call `pages[i].SetRenderedBody([]byte(html))`. `frontMatter`, `url`, and `path` in the return are ignored — they are read-only context for conditional processing. **HTML-only filtering (issue #1102):** In `Build()`, the `onPageRendered` dispatch block must skip pages whose `Outputs` does not include `"html"`. Create a helper function `pageHasHTMLOutput(page *content.Page) bool` that returns true when `page.Outputs` is nil/empty or contains `"html"`. When building the payloads slice for `RunBatchWithProgress`, only include pages where `pageHasHTMLOutput` returns true. Maintain an index mapping so results are applied back to the correct pages. **Edge-case handling (issue #1120):** The inline apply-back block (`toGoMap(result)` + `m["html"].(string)`) handles four edge cases gracefully: (1) nil/undefined result → no-op, original HTML preserved, no warning; (2) map without `html` key → original preserved, migration warning logged; (3) map with non-string `html` → original preserved, migration warning logged; (4) raw string return (old API) → original preserved, migration warning logged. **Dead code cleanup (issue #1143):** The standalone `extractPageRenderedHTML` function in `hooks.go` is dead code — both `Build()` and `BuildIncremental()` inline the `toGoMap` + `m["html"]` extraction directly (inlined during PR #1137 to access the full result map for `extractAddDependencies`). The developer must remove `extractPageRenderedHTML` from `hooks.go`. `buildPageRenderedPayload` must call `convertOrderedMaps(page.FrontMatter)` to recursively flatten `*ordered.Map` values to `map[string]interface{}` at all nesting levels. Both `Build()` and `BuildIncremental()` must produce identical payload shapes with all four fields correctly populated — including updated front matter values on incremental rebuilds.
  - **`onFormatRendered`** (issue #1102): fires once per non-HTML format body after template rendering. The developer must:
    1. **Add constant**: Add `OnFormatRendered HookName = "onFormatRendered"` to `internal/plugin/hooks.go` alongside the existing hook constants.
    2. **Add payload struct**: Create `HookFormatRenderedPayload` in `internal/plugin/payload.go` with five fields: `Format string \`json:"format"\``, `Content string \`json:"content"\``, `URL string \`json:"url"\``, `Path string \`json:"path"\``, `FrontMatter map[string]interface{} \`json:"frontMatter"\``.
    3. **Add `FormatContent` to `BuildResult`**: Add `FormatContent map[string]map[string]string` (page key → format → rendered content) to `BuildResult` in `internal/pipeline/build.go`. Populate it alongside `RenderedContent` when `CaptureRenderedContent` is true — iterate all pages, and for each page with non-empty `FormatBodies`, copy each entry as `string(body)` keyed by `renderedContentKey(page)` then format.
    4. **Build helper functions**: Create `buildFormatRenderedPayload(page *content.Page, format string, content string, fm map[string]interface{}) HookFormatRenderedPayload` in `internal/pipeline/hooks.go` — the caller pre-converts front matter once per page via `convertOrderedMaps(page.FrontMatter)` and passes it in as the `fm` parameter to avoid repeated conversion across hooks × formats. Create `extractFormatRenderedContent(result interface{}) (string, bool)` — uses `toGoMap(result)` + `m["content"].(string)` to extract the `"content"` key.
    5. **Pipeline integration in `Build()`**: After the `onPageRendered` dispatch block, add an `onFormatRendered` dispatch block. Iterate all pages: for each page, skip pages with no `FormatBodies`. Iterate `page.Outputs` (the ordered slice, not the `FormatBodies` map) to ensure deterministic dispatch order — skip `"html"` entries, then look up `page.FormatBodies[format]` for each remaining format string. For single non-HTML pages, dispatch the primary `page.RenderedBody` when `page.Outputs` is non-empty and does not contain `"html"` (using `page.Outputs[0]` as the format string). For each format body, build a `HookFormatRenderedPayload` and dispatch through `ps.Hooks.RunEachWithTimeout(plugin.OnFormatRendered, ...)`. Apply back: if `extractFormatRenderedContent` returns a string, replace `page.FormatBodies[format]` (or `page.SetRenderedBody` for single non-HTML pages).
    6. **Edge-case handling**: Mirror `onPageRendered` edge cases — nil/undefined return preserves original content, map without `content` key preserves original, non-string `content` value preserves original. No migration warning is needed (this is a new hook with no legacy format).
    7. **`BuildIncremental` parity**: `onFormatRendered` must fire during `BuildIncremental` with the same dispatch logic, payload shape, and apply-back behavior as `Build()`.
    8. **Test coverage**: 14 spec tests in `internal/pipeline/hooks_test.go` covering: onPageRendered still fires for HTML (default), onPageRendered skips JSON-only pages, onPageRendered fires HTML on multi-format pages, onPageRendered fires when HTML is non-first in outputs array, onFormatRendered payload shape validation, content mutation applied, nil return preserves original, return without content key preserves original, does not fire for HTML-only pages, single JSON page fires onFormatRendered with write-back assertion, multiple formats fire once each, read-only fields ignored with frontMatter immutability verification, E2E dual-hook test, non-string content preserves original.
...
- **Typed outbound payload structs (issue #529)**: The outbound serialization path (Go → Plugin) must use typed structs with json tags instead of `map[string]interface{}`. This separates the plugin bridge serialization path from the template rendering path (which uses `map[string]interface{}` for liquidgo). Typed structs allow `sonic.ConfigStd` to JIT-compile optimized encoders for fixed fields — only `FrontMatter` and `SiteData` (user-defined keys) require reflection. The inbound path (Plugin → Go) remains `map[string]interface{}` since plugin return shapes are dynamic. Structs are defined in `internal/plugin/payload.go`:
  ...
  - **QuickJS native object fast path (issue #1180)**: The `default` case in `CallHook`'s type switch JSON-serializes the entire payload struct including large HTML/content string fields. For `HookRenderedPayload` (~800KB avg HTML on RHDS), this causes 4 serialization passes per page: `jsonCodec.Marshal` → `JSON.parse` → `JSON.stringify` → `ordered.UnmarshalJSONValue` = ~3.2MB of JSON churn per call. The fix adds type-specific `case` branches that build JS objects directly via the QuickJS API, transferring large string fields as native strings (same zero-serialization path as the old `case string:` branch). Three payload types need fast paths:
    - `case HookRenderedPayload:` — `html`, `url`, `path` via `r.ctx.NewString()`; `frontMatter` via JSON-serialize + `JSON.parse` (small map, typically <1KB)
    - `case HookFormatRenderedPayload:` — `content`, `format`, `url`, `path` via `NewString()`; `frontMatter` via JSON
    - `case HookTransformPayload:` — `html`, `url`, `path` via `NewString()`; `frontMatter` via JSON; `toc` via JSON (small array)
    For each case: create a JS object via `r.ctx.NewObject()`, set string properties via `obj.SetPropertyStr(key, r.ctx.NewString(val))`, JSON-serialize only the small non-primitive fields (`frontMatter`, `toc`) into a temp global, `JSON.parse` them in the VM, and set the parsed result as a property on the object. Set the assembled object as `__callInput`. The hook-call evaluation remains unchanged — `__hooks[__callHookName](__callInput)` receives the same JS object shape.
  ...
  - **Pre-compiled JS helpers (issue #1186)**: `invokeHookFastPath` previously called `ctx.Eval("hook-call.js", qjs.Code("__hooks[__callHookName](__callInput)"))` per page per hook — the QuickJS `Eval` method lexes, parses, and compiles the source string on every call. For 820 pages × N hooks, that is 820×N redundant compilations of the same static expression. The fix pre-compiles three global JS functions in `Init()` and invokes them by name on hot paths:
    1. **Add pre-compiled functions to Init()**: In a separate `r.ctx.Eval("precompile.js", ...)` call after the alloy object setup, define three global functions. The lazy getter installers (`__installLazyFM`, `__installLazyTOC`) use the #1187 Go-callback approach — they take only `target` (no `jsonStr` param) and defer serialization to `__resolveFM()`/`__resolveTOC()` Go callbacks registered via `r.ctx.SetFunc`:
       ```javascript
       ...
       ```
       The self-replacing pattern (`Object.defineProperty(this, ...)` inside the getter) converts the accessor to a data property on first read, caching the parsed result. The setter allows `page.frontMatter = {...}` without TypeError.
    2. **Invocation mechanism**: Functions are invoked by name at call sites — no cached `*qjs.Value` references are stored on `QuickJSRuntime`. `__callHookByName` is called via `r.ctx.Global().InvokeJS("__callHookByName", nameVal, input)`. The lazy getter installers (`__installLazyFM`, `__installLazyTOC`) are retrieved per-call via `r.ctx.Global().GetPropertyStr("__installLazyFM")`, called via `r.ctx.Invoke(installFn, r.ctx.Global(), obj)`, and freed immediately via `defer installFn.Free()`. This avoids managing `*qjs.Value` lifetimes across the runtime's lifetime and requires no explicit `Free()` calls in `Close()`.
    3. **Hot path replacements**:
       - `invokeHookFastPath`: Uses `r.ctx.Global().InvokeJS("__callHookByName", nameVal, input)` — the input JS object is passed directly as a function argument. No globals (`__callInput`, `__callHookName`) are set or cleaned up.
       - `setPayloadFrontMatter`: Sets `r.pendingFM = fm`, retrieves `__installLazyFM` via `GetPropertyStr`, calls `r.ctx.Invoke(installFn, r.ctx.Global(), obj)`. The Go callback `__resolveFM` (registered in `Init()`) reads `r.pendingFM` and marshals to JSON on demand. Nil frontMatter installs the same lazy getter — `__resolveFM` returns `"{}"` for nil.
       - `callHookTransformPayload`: For non-empty TOC, sets `r.pendingTOC = v.TOC`, retrieves `__installLazyTOC` via `GetPropertyStr`, calls `r.ctx.Invoke(installFn, r.ctx.Global(), obj)`. Nil/empty TOC is omitted (unchanged).
    **Behavioral contract**: The lazy getter pattern is transparent to plugins. `page.frontMatter.title` triggers the getter on first access, calls the `__resolveFM` Go callback to marshal the Go map, parses the JSON, self-replaces with a data property, and returns the value. Subsequent accesses return the cached data property directly. `page.frontMatter = {...}` triggers the setter, replacing the accessor with a writable data property. `Object.keys(page)` includes `frontMatter` and `toc` because the descriptors set `enumerable: true`. The existing spec tests in the #1180 section continue to pass unchanged — they exercise the functional contract (field delivery, identity return, etc.) which is preserved. The #1186 tests verify the pre-compiled helpers exist and that frontMatter/toc use lazy getters (property descriptor has `get` function).
  ...
  - **Targeted outbound extraction (issue #1185)**: Replace the generic `extractHookResult` (iterates all own properties via `GetOwnPropertyNames` + `JSONStringify` per non-primitive property) with three type-aware extractors. Each reads only the fields the pipeline consumes from the return value:
    ...
    - `extractTransformResult(result *qjs.Value) (interface{}, error)` — reads `html` via `GetPropertyStr("html").String()`. Checks for `toc` and `addDependencies`. Skips `url`, `path`, `frontMatter`.
    The `invokeHookFastPath` function must accept a payload-type discriminator and call the correct extractor. `extractHookResult` remains as fallback for non-fast-path hooks and for backward-compat string returns.
    **Identity return tests updated**: The three identity-return spec tests now assert that read-only fields (url, path, frontMatter, format) are NOT present in the result — the targeted extractor drops them because the pipeline never reads them back. All three payload types are covered: HookRenderedPayload (html present, url/path/frontMatter absent), HookFormatRenderedPayload (content present, format/url/path/frontMatter absent), HookTransformPayload (html and toc present, url/path/frontMatter absent).
    **Pipeline-level frontMatter read-only test (issue #1201)**: 1 spec test in `internal/pipeline/hooks_test.go` exercises `fireContentTransformedHooks` directly (exported as `FireContentTransformedHooks` via `export_test.go`) with a Go hook function that returns a map containing modified frontMatter. Asserts that `page.FrontMatter` retains the original value after the hook — proving the pipeline does NOT read frontMatter back from the return. This test bypasses QuickJS/Node serialization to exercise the pipeline apply-back code path directly. The developer must remove the `modified["frontMatter"]` extraction block from `fireContentTransformedHooks` to make this test pass.
  - **Lazy frontMatter and TOC serialization (issue #1187)**: The fast path (issue #1180) still eagerly JSON-serializes `frontMatter` and `toc` on every hook call via `setPayloadFrontMatter` and inline `jsonCodec.Marshal(v.TOC)` in `callHookTransformPayload`. Most post-render hooks never read these properties — on the RHDS benchmark (820 pages), frontMatter serialization alone costs ~3.2 seconds. The fix replaces eager serialization with lazy `Object.defineProperty` accessors backed by Go callbacks that only marshal on demand:
    1. **New fields on `QuickJSRuntime`**: Add `pendingFM map[string]interface{}` and `pendingTOC []content.TOCEntry` to hold the Go data for the current hook call. These are set before each hook invocation and read by the Go callbacks.
    2. **Go callbacks registered in `Init()`**: Register `__resolveFM` via `r.ctx.SetFunc` — reads `r.pendingFM`, marshals to JSON via `jsonCodec.Marshal`, returns the JSON string. If `r.pendingFM` is nil, return `"{}"` (empty object). Register `__resolveTOC` via `r.ctx.SetFunc` — reads `r.pendingTOC`, marshals to JSON via `jsonCodec.Marshal`, returns the JSON string.
    3. **Pre-compiled JS installer functions in `Init()`**: Compile `__installLazyFM(target)` — calls `Object.defineProperty(target, 'frontMatter', {get, set, configurable: true})`. The getter calls `JSON.parse(__resolveFM())` then self-replaces with a data property (`Object.defineProperty(this, 'frontMatter', {value: v, writable: true, configurable: true})`). The setter replaces the accessor with a writable data property holding the written value — required because `page.frontMatter = {...}` must work without TypeError. Compile `__installLazyTOC(target)` — identical pattern with `__resolveTOC`.
  ...
  - **Hook chain `map[string]interface{}` fast path (issues #1185, #1188, #1202)**: When hooks chain (first hook's result feeds second hook), the payload is `map[string]interface{}` not a typed struct. Add a `case map[string]interface{}:` branch to the `callHookLocked` type switch. Determine the payload type from the **hook name** (the `name` parameter), not from map key contents: `"onPageRendered"` → `payloadRendered`, `"onFormatRendered"` → `payloadFormatRendered`, `"onContentTransformed"` → `payloadTransform`. For known hook names: build a JS object directly via `NewObject()`/`SetPropertyStr()` — same approach as the typed struct cases, then pass through `callHookMapPayload` with the correct `payloadType`. For unknown hook names: fall through to the `default` JSON serialization path (same as before for non-page maps like `onBuildComplete`, `onDataFetched`). The previous key-content heuristic (disambiguating `payloadTransform` vs `payloadRendered` by `toc` key presence) must be replaced because it fails when a plugin returns extra keys — e.g., `onPageRendered` returning `{html, toc}` triggers misclassification as `payloadTransform` (issue #1202). The Node IPC path retains key-based detection (unchanged by this refactor) in `splitBodyField` in `node.go`: `map[string]interface{}` with `html` or `content` → split-body framing; without → standard JSON framing.
    **Test coverage**: 11 spec tests — QuickJS: chain receives html from first hook's map result, chain receives content from first hook's map result (onFormatRendered scenario), non-page map falls through to JSON path, onDataFetched map payload NOT caught by page-like fast path (issue #1188), onPageRendered map with toc key uses rendered extraction not transform (issue #1202), onContentTransformed map without toc key uses transform extraction (issue #1202), onFormatRendered map with html key uses format-rendered extraction not rendered (issue #1211), onContentTransformed map with content key but no html uses transform extraction not format-rendered (issue #1212). Node IPC: standard single-header framing for hook map without html/content keys (issue #1188). Pipeline end-to-end: two onPageRendered hooks chain through build with both modifications in output, two hooks chain with large HTML through build without truncation or corruption (issue #1188).
  - **Hook chain context preservation (issue #1216)**: When multiple hooks chain on the same event (different priorities), the second hook must receive the full payload context (`url`, `path`, `frontMatter`) — not just the mutable field (`html`/`content`). The outbound extractors (`extractRenderedResult`, `extractTransformResult`, `extractFormatRenderedResult`) currently strip context fields because the pipeline doesn't consume them on return, but chained hooks need them for conditional processing. **Fix**: After extraction, merge `url` and `path` as strings from the original input payload into the result map. For `frontMatter`, do not copy the Go map — instead set `r.pendingFM` to the original payload's FM value and have `callHookMapPayload` install `__installLazyFM` (the same lazy getter the typed struct path uses). Net cost per chain step: two string map assignments + one `ctx.Invoke` for the lazy getter installer. FM is only serialized if the next hook reads `page.frontMatter`. **Read-only semantics**: `url`, `path`, and `frontMatter` in the chained result must reflect the original input payload, not the hook's return. If a hook returns `{ url: "/mutated/" }`, the chaining layer carries forward the original `url`. **Scope**: `onPageRendered` (chains via `RunBatchWithProgress`) and `onContentTransformed` (chains via `RunWithTimeout`) are affected. `onFormatRendered` is immune — it uses `RunEachWithTimeout` with per-hook payload reconstruction via `payloadFn`.
    **Test coverage**: 4 spec tests — `onPageRendered` chain preserves url/path/frontMatter for second hook, `onContentTransformed` chain preserves url/path/frontMatter for second hook, `onPageRendered` url mutation in hook return does not propagate to next hook (read-only semantics), `onFormatRendered` chain preserves full context (regression guard, passes green).
  - **Hook chain context preservation: cross-runtime chains (issue #1219)**: `injectChainContext` only handles `map[string]interface{}` results (from QuickJS). WASM's `CallHook` returns `*ordered.Map` via `ordered.UnmarshalJSONValue` and Node's `CallHook` returns `*ordered.Map` via `ordered.RewrapValue`. When these results chain, the type assertion `result.(map[string]interface{})` fails silently, context is not injected, and subsequent hooks lose `url`, `path`, and `frontMatter`. **Fix**: Add an `*ordered.Map` branch to `injectChainContext` in `hooks.go` that calls `om.Set("url", cc.pageURL)`, `om.Set("path", cc.pagePath)`, `om.Set("frontMatter", cc.fm)` — mirroring the `map[string]interface{}` path. The same read-only semantics apply: original context always wins. Import `"github.com/zeroedin/alloy/internal/ordered"` in `hooks.go`. **Performance note**: When a WASM/Node `*ordered.Map` result chains into a QuickJS hook, `callHookLocked` falls through to the JSON slow path (no `case *ordered.Map:` branch). This is correct — the injected context fields serialize normally — but loses the lazy frontMatter getter optimization. A `case *ordered.Map:` branch in `callHookLocked` that converts to `map[string]interface{}` via `om.ToGoMap()` and routes to `callHookMapPayload` would restore the optimization; this is optional and not required for correctness.
    **Test coverage**: 6 spec tests in `hooks_test.go` — `Run` injects url/path/frontMatter into `*ordered.Map` result (onPageRendered), `RunWithTimeout` injects context into `*ordered.Map` result (onContentTransformed), `RunBatchWithProgress` batch path injects per-item context into `*ordered.Map` results, `RunBatchWithProgress` per-item fallback injects context into `*ordered.Map` result, `*ordered.Map` url/path mutation in hook return does not propagate (read-only semantics), mixed chain with `map[string]interface{}` then `*ordered.Map` preserves context through both types.
  - **`BatchCallHook` on `QuickJSRuntime` (issues #1185, #1190)**: New method `func (r *QuickJSRuntime) BatchCallHook(name string, payloads []interface{}, onProgress func(int)) ([]interface{}, error)`. Loops synchronously through payloads calling `CallHook` internally, calling `onProgress(i+1)` after each item. No per-item goroutine/channel/context overhead. On error, the method must wrap with `fmt.Errorf("item %d: %w", i, err)` to identify which payload failed (0-based index). Registered via the existing `registerRuntime` batch detection (type assertion for `BatchCallHook` method in `registry.go`).
    **Test coverage**: 5 spec tests — results match sequential `CallHook` calls for 3 payloads, onProgress callback receives 1-based indices [1, 2, 3], empty payloads return empty (non-nil) slice and nil error, error propagation includes 0-based item index and original JS error message (issue #1190), `registerRuntime` detects `BatchCallHook` interface and registers hooks with batch functions (issue #1190).
  - **Pre-compiled JS functions (issue #1185)**: During `Init()`, compile three global JS functions via `r.ctx.Eval("precompile.js", ...)`. The functions are defined as globals and called by name at invocation sites — no `*qjs.Value` references are cached on the struct:
    - `__callHookByName` — `function(name, input) { return __hooks[name](input); }`. Called via `r.ctx.Global().InvokeJS("__callHookByName", ...)`. Replaces per-call `Eval("__hooks[__callHookName](__callInput)")` which parses+compiles 820 times per hook on RHDS.
    - `__installLazyFM` — the `Object.defineProperty` installer for frontMatter lazy getter (uses `__resolveFM()` Go callback). Retrieved per-call via `GetPropertyStr`, called via `ctx.Invoke`, freed immediately.
    - `__installLazyTOC` — same pattern for toc (uses `__resolveTOC()` Go callback).
    **Test coverage**: 1 spec test — multiple sequential CallHook calls produce correct results with incrementing closure state.
  - **`Page.SetRenderedHTML(string)` (issues #1185, #1189)**: New method on `content.Page` in `internal/content/page.go`. Stores both `p.RenderedBody = []byte(s)` and `p.renderedStr = s` simultaneously. Avoids the round-trip: `SetRenderedBody([]byte(html))` clears `renderedStr`, then next `HTML()` call re-converts `[]byte→string`. Pipeline hook apply-back should call `SetRenderedHTML` instead of `SetRenderedBody([]byte(...))` when the source is already a string (which it always is from QuickJS `GetPropertyStr().String()`).
    **Test coverage**: 6 spec tests in `internal/content/page_set_html_test.go` — basic contract (HTML returns correct string), RenderedBody matches, overwrites previous, empty string, restores after release, and SetRenderedBody-after-SetRenderedHTML cache invalidation.
  - **`convertedFrontMatter(page)` caching (issue #1185)**: New function in `internal/pipeline/context.go` (or `hooks.go`). Replaces `convertOrderedMaps(page.FrontMatter)` at call sites. Implementation: `func convertedFrontMatter(page *content.Page) map[string]interface{} { fm := convertOrderedMaps(page.FrontMatter); page.FrontMatter = fm; return fm }`. The key property: `convertOrderedMaps` already returns the input map unchanged when `needsOrderedMapConversion` returns false (no `*ordered.Map` values). After the first call stores the converted map back, subsequent calls find no `*ordered.Map` values and return immediately. Replace all 6 `convertOrderedMaps(page.FrontMatter)` call sites in `hooks.go` with `convertedFrontMatter(page)`.
    **Test coverage**: 5 spec tests in `internal/pipeline/converted_fm_test.go` — converts ordered.Map on first call, stores back on page, second call returns same result, nil FrontMatter, no-op for plain maps. Exported via `pipeline.ConvertedFrontMatter` in `export_test.go`.
...
- **WASM runtime (issue #181)**: `WASMRuntime.LoadModule()` uses wazero to compile and instantiate the WASM binary, discover exported functions (`alloc`, `filter`, `shortcode`, `hook`, `hooks`), and register them. `alloc` export is required — used by the host to get a safe write offset in WASM linear memory (issue #186). `CallExport(name, args...)` must call `alloc(inputLen)`, write input bytes at the returned pointer, call the export with `(ptr, len)`, and read the result from the returned packed `i64` (issue #190). `RegisteredFilters()` must return filter names from the module's exports. `LoadModule` must return an error for invalid WASM binaries. See PLAN.md §5 WASM Calling Convention for full ABI.
- **WASM packed i64 ABI (issue #1222)**: All data-returning WASM exports use a packed `i64` return convention: `result = (ptr << 32) | len`. The host unpacks as `ptr = uint32(result >> 32)`, `len = uint32(result & 0xFFFFFFFF)`. This replaces the previous multi-value `(result i32 i32)` form, which is not producible by Rust ≥ 1.82, TinyGo, or AssemblyScript without unstable/unsupported compiler flags. The packed form is the natural output of all three toolchains (`-> u64` in Rust, `uint64` in TinyGo, `u64` in AssemblyScript).
  - **Call sites to update**: Four functions in `wasm.go` read `(ptr, len)` pairs from WASM results and must change from `results[0]`/`results[1]` multi-value reads to packed `i64` unpacking:
    1. **`callStringFilter`**: Change `resultPtr := uint32(results[0])` / `resultLen := uint32(results[1])` to `resultPtr := uint32(results[0] >> 32)` / `resultLen := uint32(results[0] & 0xFFFFFFFF)`. Remove the `len(results) >= 2` guard — packed always returns exactly one result. The `resultPtr == 0 && resultLen == 0` error check works unchanged after unpacking.
    2. **`CallExportRaw`**: Same unpacking change. Remove the `len(results) >= 2` guard.
    3. **`discoverHooks`**: `hooks()` is a zero-arg export returning packed `i64`. Change `ptr := uint32(results[0])` / `length := uint32(results[1])` to unpack from `results[0]`. Remove the `len(results) < 2` guard.
    4. **`last_error` reads**: Four inline `last_error()` call sites (one each in `callStringFilter`, `CallExportRaw`, `discoverHooks`, and `discoverHooks` error path) read `errResults[0]`/`errResults[1]`. All four must unpack from a single `errResults[0]`. Remove the `len(errResults) >= 2` guards.
  - **`CallExport` zero-arg fallback removal (issue #1222)**: The zero-arg fallback branch at the bottom of `CallExport` (the `if len(args) == 0` path that calls `fn.Call()` and returns `results[0]` raw) is dead code — no production caller reaches it, and the zero-arg exports (`hooks()`, `last_error()`) are called directly on the wazero API. Remove this branch. `CallExport` must require at least one argument and return an error if called with none.
  - **Load-time signature validation (issue #1222)**: After module instantiation in `LoadModule`, inspect every data-returning export's signature via `fn.Definition().ResultTypes()` and `fn.Definition().ParamTypes()`. The valid signatures are:
    - `filter`, `shortcode`, `hook`: `params=[i32, i32]`, `results=[i64]`
    - `hooks`, `last_error`: `params=[]`, `results=[i64]`
    If an export has a non-conforming signature (e.g., the old `(result i32 i32)` multi-value form, the sret `(param i32 i32 i32) -> ()` form, or any other shape), `LoadModule` must return an error with a diagnostic naming the export, its actual signature, and suggesting the packed `i64` form. Use `api.ValueTypeI64` and `api.ValueTypeI32` constants from wazero's `api` package for comparison.
  - **Error convention**: Packed `0` is equivalent to `(ptr=0, len=0)`. The existing `resultPtr == 0 && resultLen == 0` check works unchanged after unpacking, since `uint32(0 >> 32) == 0` and `uint32(0 & 0xFFFFFFFF) == 0`.
  - **Test fixtures (issue #1222)**: All 11 WAT fixtures in `testdata/single-files/` must be updated from `(result i32 i32)` to `(result i64)` with the packed shift/or return sequence. Three new WAT fixtures must be added: `wasm-old-multivalue-abi.wat/.wasm` (uses the old `(result i32 i32)` form, for testing load-time rejection), `wasm-sret-abi.wat/.wasm` (uses the sret `(param i32 i32 i32) -> ()` form, for testing load-time rejection), and `wasm-last-error.wat/.wasm` (exports `filter` returning packed 0 and `last_error()` returning a packed i64 pointing to a static error message, for testing packed `last_error()` unpacking). All WAT sources are compiled with `wat2wasm` — no toolchain dependency in CI.
  - **Build-failure propagation (issue #1224)**: PLAN.md §5 error handling says "build aborts in `alloy build`." The packed i64 ABI change implements error returns from `callStringFilter`/`CallExportRaw`/`discoverHooks` but does not wire build-abort behavior — that is tracked separately in issue #1224 (filter/hook call errors must fail the build). The developer should implement correct error returns from the WASM call sites but not add build-abort logic in this scope.
  - **Test coverage (issue #1222)**: 10 spec tests in `internal/plugin/wasm_test.go` under "WASM packed i64 ABI (issue #1222)": packed filter returns transformed string, packed filter is not passthrough (disambiguation), packed error convention (packed `0` returns error), packed `last_error()` returns error message string, packed `hooks()` discovers hook names, packed `hook()` processes payload and preserves content, `CallExport` with zero args returns error, load-time rejection of old multi-value `(result i32 i32)` ABI with diagnostic mentioning actual signature, load-time rejection of sret `(param i32 i32 i32)` ABI with diagnostic mentioning param count, and one end-to-end pipeline test exercising a packed-ABI WASM plugin through Registry → LoadPlugins → CallFilter. Test fixture `wasm-last-error.wasm` exports `filter` (always returns packed 0) and `last_error()` (returns packed i64 pointing to a static error message).
- **WASM hook support (issue #444)**: `WASMRuntime` must support hook registration and execution via the `hooks()`/`hook()` export ABI:
  - **Hook discovery**: `LoadModule` must check for a `hooks` export after module instantiation. If present, call it (no arguments — zero-arg function returning packed `i64`, same pattern as `last_error`), unpack `ptr = result >> 32`, `len = result & 0xFFFFFFFF`, read the returned bytes as a JSON array. Elements can be plain strings (backward compat) or objects with metadata. Store results in `r.hookRegistrations []HookRegistration` alongside `r.hookNames []string` (derived from registrations for `RegisteredHooks()` compat). If no `hooks` export, both fields remain nil. Add `hooks` to `wasmRuntimeExports` (note: `hook` is already reserved but `hooks` is not — add it).
  ...
  - **Hook error handling**: Three error paths:
    1. **`hook()` returns packed `0`** (equivalent to ptr=0, len=0): Same convention as `filter`/`shortcode`. `CallHook` must check `last_error()` if available and return an error containing the module's error message. No silent fallback to original payload.
...
- **Unified plugin bridge (issues #189, #237)**: All plugin tiers (Tier 2 QuickJS, Tier 2 WASM, Tier 3 Node) must implement the same `Runtime` interface. `Registry.Runtimes()` returns `[]Runtime` and the bridging loop in `Registry.LoadPlugins()` works identically for all tiers — no tier-specific code.
  ...
  - **`NodeRuntime`** — subprocess execution via JSON-RPC over stdin/stdout (length-prefixed, LSP-style framing). Stderr connected to Alloy's own stderr (the user's terminal). No log file is created — `console.log` debugging works the way JS developers expect.
  - **Worker pool (issue #491)**: For per-page hooks (`onPageRendered`, `onContentTransformed`, `onFormatRendered`), spawn multiple subprocess workers and distribute pages across them. Each worker loads the same plugins via `EvalFile`. Implementation:
...
- **Plugin data source wiring (issue #979)**: `alloy.source(name, fn)` is the JS API for registering data source handlers. Tier 3 (Node) only — source plugins need network access, npm packages, and environment variables. The implementation connects three layers: bridge registration, Go-side handler wrapping, and pipeline dispatch.
  ...
  **NodeRuntime specifics:**
  ...
  - **Plugin output goes to terminal, not a log file (issue #1220)**: `cmd.Stderr` is set to `os.Stderr`, so plugin `console.log`/`warn`/`info`/`debug` output and `process.stdout.write` (patched to stderr by the bridge) all appear in the user's terminal alongside Alloy's build output. No `.alloy/plugin.log` file is created. Remove the dead `LogPath()` method from `NodeBridge` — it has no production caller.
  ...
  - **Node IPC binary framing (issue #1181)**: `EncodeMessage` must detect hook messages with large-body payload types and use split-body framing to avoid JSON-encoding large HTML/content strings. Implementation:
    1. **`EncodeMessage` outbound split-body**: Add detection logic that checks `msg.Type == "hook"` and does a type switch on `msg.Payload`. For `HookRenderedPayload` (extract `HTML` as field name `html`), `HookFormatRenderedPayload` (extract `Content` as field name `content`), `HookTransformPayload` (extract `HTML` as field name `html`), or `map[string]interface{}` with `html`/`content` string key (check `html` first): extract the large field value, build a modified payload without it (marshal the struct to JSON, unmarshal to `map[string]interface{}`, delete the key; for map payloads, shallow-copy the map and delete — do not mutate the caller's map), set `msg.Payload` to the modified map, marshal the full Message to JSON, and emit a multi-header frame: `Content-Length: <json-len>\r\nX-Body-Length: <body-len>\r\nX-Body-Field: <field-name>\r\n\r\n<JSON><raw-bytes>`. For all other messages or when `msg.Type != "hook"`: use the existing single-header format unchanged.
    2. **`DecodeMessage` inbound split-body parsing**: After `SplitN(raw, "\r\n\r\n", 2)`, parse the header block (split on `\r\n`) for `X-Body-Length` and `X-Body-Field` in addition to `Content-Length`. If both split-body headers are present: the first `Content-Length` bytes of `parts[1]` are the JSON body, the next `X-Body-Length` bytes are the raw body. Unmarshal the JSON portion into Message, then type-assert Result (or Payload if Result is nil) to `map[string]interface{}` and inject the raw body string under the `X-Body-Field` key. If the split-body headers are absent: parse the entire `parts[1]` as JSON (backward compat, unchanged).
    3. **`NodeBridge.Send` multi-header response reader**: Replace the current single-`ReadString('\n')` header reading with a header-reading loop: read lines via `ReadString('\n')` and `strings.TrimSpace` until a blank line is found. The first line must still be checked for the `Content-Length:` prefix — non-`Content-Length` data on the first line triggers the stdout pollution diagnostic (existing malformed frame error, unchanged). Parse `Content-Length`, `X-Body-Length`, and `X-Body-Field` from the header lines. Read `Content-Length` bytes as JSON body via `io.ReadFull`. Parse JSON into `resp Message`. If `X-Body-Length` is present, read `X-Body-Length` additional bytes via `io.ReadFull`, type-assert `resp.Result` to `map[string]interface{}`, and inject the raw body string under `X-Body-Field`. If `X-Body-Length` is absent, behave as before (backward compat).
    4. **`bridge.js` inbound parser**: Update the `process.stdin` `data` handler's header parsing. After finding `\r\n\r\n`, parse the header block for `Content-Length`, `X-Body-Length`, and `X-Body-Field` (regex or string split). If split-body headers are present: slice `Content-Length` bytes from the body as JSON, parse it, then slice `X-Body-Length` bytes as raw body, inject into `msg.payload[bodyField]` or `msg.result[bodyField]` depending on which is an object. If not: parse as before. The `buffer.length < bodyStart + len` overflow guard must account for both Content-Length and X-Body-Length when split-body headers are present.
    5. **`bridge.js` outbound `sendMessage`**: For hook result messages, check if the result object has an `html` or `content` string field (check `html` first). If yes: create a shallow clone of the result object, delete the field from the clone, `JSON.stringify` the modified message (with the clone as result), compute `Buffer.byteLength` for both the JSON and the raw field value, write the multi-header frame via `realStdoutWrite`: `Content-Length: <json-len>\r\nX-Body-Length: <body-len>\r\nX-Body-Field: <field-name>\r\n\r\n<JSON><raw-bytes>`. If no: use the existing single-header format. The hook/non-hook determination requires the inbound message type — pass it through to `sendMessage` or use a response wrapper that carries the original request type.
    6. **Test coverage**: 19 spec tests in `internal/plugin/node_test.go` under "Split-body binary framing (issue #1181)": EncodeMessage split-body for HookRenderedPayload (html), HookFormatRenderedPayload (content), HookTransformPayload (html), map with html key (chained result), map with content key (chained onFormatRendered), multi-byte characters (byte-length vs rune-count validation), EncodeMessage preserves single-header for non-hook filter message and empty split field, dual-key map priority (html over content when both exist), caller map immutability guard (EncodeMessage must not delete from caller's map), DecodeMessage parses multi-header frame and injects raw body into result, Send reads multi-header response and injects raw body into result. Edge case tests (issue #1194): EncodeMessage→DecodeMessage round-trip preserves split field in payload, DecodeMessage rejects truncated frame (Content-Length exceeds available bytes), DecodeMessage rejects negative X-Body-Length, DecodeMessage rejects X-Body-Length without X-Body-Field, Send rejects stdout pollution in key:value format, Send rejects negative X-Body-Length, Send rejects missing X-Body-Field. Backward compatibility and malformed frame diagnostic are validated by existing tests (13 tests across "Message framing", "Message types", and "Malformed frame diagnostic" sections). All JSON assertions in the test file use `jsonutil.JSON` (bytedance/sonic) instead of `encoding/json` (issue #1195) to match production codec behavior.
...
- **Plugin dependency tracking: addDependencies + invalidateByDependency (issue #1100)**: Enables targeted incremental rebuilds for content pages whose output depends on external files (SSR components, Sass partials, translation files). The developer must implement:
  1. **Cache** (`internal/cache/cache.go`): Add a `pageDependencies map[string]map[string]bool` field (dependency path → set of page RelPaths). Initialize in `New()`. Add `TrackDependency(pageRelPath, depPath string)` following the `TrackVirtualDependency` pattern (reverse map insertion). Add `PagesForDependency(depPath string) []string` following the `InvalidatedVirtualPages` pattern (lookup + collect, nil when not tracked). Add `ClearDependencies()` to reset only `pageDependencies` without affecting hashes, templates, or virtual page tracking. Update `Clone()` to deep-copy `pageDependencies`. Update `Clear()` to reset `pageDependencies`.
  2. **addDependencies extraction from onPageRendered** (`internal/pipeline/build.go`, `internal/pipeline/incremental.go`): In the `onPageRendered` apply-back loop (both `Build()` and `BuildIncremental()`), after extracting `html` from the result via `toGoMap`, also check for an `addDependencies` key. If present and is `[]interface{}`, iterate entries: string entries are valid dependency paths; non-string entries produce a `log.Printf` warning and are skipped. For each valid dependency path, call `buildCache.TrackDependency(pages[i].RelPath, depPath)`. Non-array `addDependencies` (e.g., string, number) produces a warning and is ignored. Empty array is a no-op. Dependencies must be cleared before re-tracking on each build — call `buildCache.ClearDependencies()` once at the start of the per-page rendering sequence, before `fireContentTransformedHooks` (which fires before `onPageRendered`), in both `Build()` and `BuildIncremental()`. Clearing before `onPageRendered` would erase dependencies already tracked by `onContentTransformed`.
  3. **addDependencies extraction from onContentTransformed** (`internal/pipeline/hooks.go`): In `fireContentTransformedHooks`, after extracting `html` and `toc` from the result (frontMatter is read-only — not extracted, issue #1201), also check for `addDependencies`. Same extraction logic as `onPageRendered`: iterate `[]interface{}`, filter strings, warn on non-string entries. Since `fireContentTransformedHooks` does not have direct access to the build cache, the developer must add a `cache *cache.Cache` parameter (or a callback) to `fireContentTransformedHooks`. Alternatively, pass the cache via `PipelineState` which is already accessible in the pipeline. For each valid dependency, call `cache.TrackDependency(page.RelPath, depPath)`.
  4. **BuildIncremental dependency-aware page invalidation** (`internal/pipeline/incremental.go`): After the content-hash comparison loop that builds `renderRelPaths`, add a dependency invalidation pass. For each `changedFile` in `changedFiles`, call `previousCache.PagesForDependency(filepath.ToSlash(changedFile))`. If the result is non-nil, add each returned page RelPath to `renderRelPaths` (use a set to avoid duplicates). This ensures pages whose external dependencies changed are re-rendered even if their own content hash is unchanged. This check runs alongside the existing `InvalidatedPages` (template invalidation) and `InvalidatedVirtualPages` checks.
  5. **ParseFileChangedResult** (`internal/plugin/file_changed.go`): Create a `FileChangedResult` struct with fields `InvalidateByDependency []string`, `Restart bool`, and `Warnings []string`. Create `ParseFileChangedResult(result interface{}) *FileChangedResult` that extracts structured data from the `onFileChanged` hook's return value. Type-assert to `map[string]interface{}` (non-map → nil). Extract `invalidateByDependency` as `[]interface{}` → filter to strings (non-string entries produce a warning, not an error). Extract `restart` as `bool` (default false); non-boolean `restart` values (numeric, string, nil) must default to false AND produce a warning in `parsed.Warnings` — JS-truthy values like `1` or `"yes"` fail Go's bool type assertion and plugin authors need a diagnostic to know their value was dropped. Return nil if neither recognized field key is present in the map (forward-compatible no-op). When a recognized key IS present but its value is malformed (e.g., `invalidateByDependency` is a string instead of an array, or `restart` is a non-boolean as the sole key), return non-nil with a warning — returning nil would silently swallow the malformed value. Unknown keys are ignored.
  6. **Dev server integration** (`cmd/dev.go`): After calling `hooks.RunWithTimeout(plugin.OnFileChanged, events)`, parse the result via `plugin.ParseFileChangedResult(result)`. If non-nil: (a) If `parsed.Restart` is true, restart the Node bridge subprocess — call `registry.Close()`, re-discover plugins via `pipeline.DiscoverPlugins(cfg)`, re-create `PipelineState`. (b) If `parsed.InvalidateByDependency` is non-empty, add those paths to the `changedFiles` slice passed to `BuildIncremental`. This causes the dependency-aware invalidation in step 4 to find matching pages. (c) Force a pipeline rebuild even if the original events were all `PassthroughChange` (recopy-only). The `needsRebuild` flag must be set to true when `invalidateByDependency` is non-empty.
  7. **onFileChanged payload update** (`cmd/dev.go`): The current code passes `events` (type `[]server.ChangeEvent`) to the hook. The developer must convert this to a serializable format matching the spec: `[]map[string]interface{}` where each entry has `"path"` (string) and `"type"` (string — the classified ChangeType name: `"content"`, `"layout"`, `"data"`, `"asset"`, `"static"`, `"component"`, `"passthrough"`, `"plugin"`). Add a `ChangeTypeString(ct ChangeType) string` function in `internal/server/watcher.go` for the conversion.
  8. **Test coverage**: 11 cache unit tests in `internal/cache/page_dependency_test.go` covering: TrackDependency + PagesForDependency roundtrip, nil for unknown path, multiple pages sharing a dependency, single page with multiple dependencies, deduplication, independence from virtual dependencies, ClearDependencies resets, ClearDependencies doesn't affect virtual deps, Clear resets page deps, Clone preservation, Clone independence. 21 plugin tests in `internal/plugin/file_changed_test.go` covering: ParseFileChangedResult extracts invalidateByDependency, empty array, non-string entries filtered with warning, restart extraction (true, false, omitted default, numeric with warning, string with warning, nil with warning — issue #1131/#1138), non-boolean restart as sole key returns non-nil with warning for numeric, string, and nil values (issue #1138 — three sole-key variants ensure key-presence check, not nil-value check), combined fields, nil/string/boolean/unrecognized-map inputs return nil, unknown keys alongside recognized keys, malformed invalidateByDependency types. 11 pipeline integration tests in `internal/pipeline/page_dependency_test.go` (helpers consolidated in outer Describe scope, no separate dep_helpers_test.go — issue #1135) covering: Build() tracks addDependencies from onPageRendered, BuildIncremental rebuilds dependency-invalidated pages, BuildIncremental skips pages with unchanged deps, dependencies re-tracked on rebuild, onContentTransformed addDependencies tracked, onContentTransformed deps trigger incremental rebuilds (issue #1133), non-array addDependencies ignored gracefully, empty array no-op, dual-hook dep accumulation from onContentTransformed + onPageRendered (issue #1132), dual-hook incremental rebuild for either dep with rebuilt cache preserving both hooks' deps (issue #1132/#1139), end-to-end dependency-invalidated rebuild.
...
- When `--config` was NOT explicitly passed (`!cmd.Flags().Changed("config")`): resolve the project directory from `--root` (or CWD), verify it exists with `os.Stat` (error: `"project directory not found: %s"`), then call `config.DetectConfigFile(dir)`. If `DetectConfigFile` returns an error, wrap it as: `fmt.Errorf("no alloy.config file found in %s (looked for alloy.config.{yaml,yml,toml,json}); use --config to specify a config file path: %w", dir, err)` — this preserves the underlying cause (e.g. permission errors) while providing actionable guidance.
...
- **Server lockfile — concurrent instance detection (issue #1094)**: Create `lockfile.go` in `internal/server/`. Detects when another `alloy dev` or `alloy serve` process is already watching the same project directory, and warns the user instead of silently corrupting output. Lockfile lives at `.alloy/server.lock` — inside the project-local cache directory, already gitignored. Uses `bytedance/sonic` via the package-level `jsonCodec` for JSON serialization (same as `watcher.go`).
  ...
  **Types and functions to create in `lockfile.go`:**
  ...
  - `RemoveLockfile(projectRoot string)` — removes `.alloy/server.lock`. No-op when the file does not exist. Does NOT remove the `.alloy/` directory — other files (fetch cache, WASM cache, profiles, worker PIDs) live there.
