...
- **Data files preserve author key order (issue #1262)**: Contract in PLAN.md → "Data files preserve author key order (issue #1262)". `LoadFileAny` must return `*ordered.Map` for YAML and TOML objects, as it already does for JSON. This supersedes the "The `LoadFile` return type stays `map[string]interface{}` for YAML/TOML" note above, for `LoadFileAny` only — `LoadFile` keeps flattening through `ToGoMap` and keeps its `map[string]interface{}` signature.
  ...
  **Reapply the four semantics the node walk bypasses.** Decoding into a `yaml.Node` skips `yaml.Unmarshal` entirely, so anything that decoder did for you is now yours to do. The first two are correctness; the last two are the safeguards `yaml.v3`'s decoder applies and a `Node` walk does not inherit. All four were found to regress a naive walk and are guarded:

  - **Duplicate mapping keys.** `yaml.Unmarshal` errors; the node walk will not. Detect a repeated key within a mapping and return a parse error naming it — `ordered.Map.Set` will not help, because it silently overwrites and the file would be accepted. **This includes a repeated `<<`**, which `yaml.Node` keeps as two separate entries in `Content` and which `yaml.Unmarshal` rejects like any other duplicate (`line 7: mapping key "<<" already defined at line 6`). Skipping merge keys before duplicate tracking accepts a malformed file and silently applies both merges. Track every key in one `seen` map and keep a separate set of non-merge keys for local-key precedence. Guarded by "still rejects duplicate mapping keys" and "rejects a duplicate merge key".
  ...
  - **Alias cycles.** `a: &a [*a]` makes the walk re-enter the node it is already expanding. This is not a slow build or a recoverable panic — it is a fatal runtime stack overflow that kills the process without naming the file. Carry an active-alias set through every branch that resolves an alias, the merge-key path included, and error as the plain decoder does (`anchor 'a' value contains itself`). Guarded by "rejects a self-referential alias instead of overflowing the stack".
  - **Alias expansion budget.** Repeated aliases expand a compact document non-linearly; `yaml.v3` bounds alias-driven work against document size and rejects the rest with `document contains excessive aliasing`. Carry `decodeCount`/`aliasCount`/`aliasDepth` counters and reuse that library's thresholds (100 aliases, 1000 decodes, the `allowedAliasRatio` curve over 400,000–4,000,000), so a document this walk accepts is one the plain decode accepted. Without it a 716-byte fixture expands toward ~10^8 nodes and the test suite hangs rather than failing. Guarded by "rejects a document whose aliases expand without bound".

  Both alias guards need shared per-document state, so the walk cannot be a set of free functions — thread a receiver through the value, mapping and merge-source paths.
  ...
  **Record implicit parents as you walk past them.** A dotted key with no table header of its own reports only its leaf: `zebra.value = 1` followed by `apple = 2` yields `zebra.value` then `apple`, with no bare `zebra` entry anywhere. A parent that is only ever an intermediate path segment therefore never gets a position, and lands in the sorted remainder behind keys written after it — `apple, zebra` for a file that reads `zebra, apple`. Append each missing parent to its level's key list while traversing. Guarded by "preserves TOML key order for dotted keys with no table header".
