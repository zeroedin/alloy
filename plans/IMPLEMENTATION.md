...
- **Render hooks (issues #273, #310, #311)**:
  ...
  **Goldmark integration**: For each hook type in `opts.Hooks`, register a custom `renderer.NodeRenderer` that builds the `markup.*` context from the AST node and calls `opts.HookRenderer(templateSrc, ctx)`. Context: `inner` from rendered children, `language` from fenced code block, `level`/`id`/`inner` (rendered HTML)/`text` (plain text)/`attributes` (goldmark-parsed `{.class #id}` map) from heading, `src`/`alt`/`title` from image, `destination`/`text`/`title`/`is_external` from link. Render hooks receive only `markup.*` — no `page.*` or `site.*` context. Heading `attributes` are extracted via `node.Attributes()` and converted to `map[string]interface{}` (issue #824). Attribute support for other block elements requires custom goldmark parser extensions (issue #892).
