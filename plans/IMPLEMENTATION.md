...
**markdown.go**:
- `RenderMarkdownWith` (issues #353, #693): Core render function accepting a pre-built goldmark instance. Signature: `RenderMarkdownWith(source []byte, md goldmark.Markdown) ([]byte, []TOCEntry, error)`. The caller creates the instance via `CreateGoldmark(opts)` once and reuses it. Always returns TOC (AST heading walk is negligible cost). `renderPages` uses `RenderMarkdownWith(body, rc.Goldmark)` for the pipeline hot path — no per-page goldmark allocation. Handles `escapeTemplateTags` internally when TemplateTags is disabled. Configure goldmark with extensions (tables, task lists, typographer, footnotes). Handle `Unsafe` (raw HTML passthrough). Handle `TemplateTags` via a custom goldmark extension (issue #564) — see **Template tag goldmark extension** below. Handle `TemplateBlocks` (issue #202) via the same extension's block parser. NO placeholder substitution — the `protectTemplateTags`/`restoreTemplateTags` functions, `blockShortcodeLineRe` regex, and all placeholder logic must be removed and replaced by proper goldmark parser extensions.
- `RenderMarkdown` (issue #693): Preserved as convenience wrapper with unchanged signature: `(source []byte, opts MarkdownOptions) ([]byte, error)`. Calls `CreateGoldmark(opts)` then delegates to `RenderMarkdownWith`, discarding TOC. Used by tests and `filters_test.go` comparisons.
- `RenderMarkdownWithTOC` (issue #693): Preserved as convenience wrapper with unchanged signature: `(source []byte, opts MarkdownOptions) ([]byte, []TOCEntry, error)`. Calls `CreateGoldmark(opts)` then delegates to `RenderMarkdownWith`. Used by tests that need TOC without managing a goldmark instance.
  ...
  **Code to remove**:
  ...
  - All placeholder logic in the render functions (formerly split across `RenderMarkdown` and `RenderMarkdownWithTOC`, consolidated into `RenderMarkdownWith` per issues #353, #693)

  **What remains unchanged**:
  - `escapeTemplateTags()` and `templateTagPattern` (used for `templateTags: false` path)
  - `CreateGoldmark()` signature and `MarkdownOptions` struct (used by convenience wrappers and callers to build the goldmark instance before passing to `RenderMarkdownWith`)
  - `RenderMarkdown()` signature `([]byte, MarkdownOptions) → ([]byte, error)` — becomes a convenience wrapper delegating to `RenderMarkdownWith` (issue #693)
  - `RenderMarkdownWithTOC()` signature `([]byte, MarkdownOptions) → ([]byte, []TOCEntry, error)` — becomes a convenience wrapper delegating to `RenderMarkdownWith` (issue #693)
...
- **Render hooks (issues #273, #310, #311)**:
  ...
  **Pipeline wiring (issues #311, #353, #693)**: In `Build()`, after `createEngine` returns, call `tmpl.DiscoverRenderHooks(layoutsDir, engineName)` once. Build `mdOpts` with hooks and `HookRenderer` closure (wraps `engine.Parse()` + `tpl.Render()`), then call `content.CreateGoldmark(mdOpts)` to create the pipeline goldmark instance. Store it on `RenderContext.Goldmark` (`goldmark.Markdown`). `renderPages` calls `content.RenderMarkdownWith(body, rc.Goldmark)` — the core function that accepts a pre-built instance (issue #693). No longer constructs `mdOpts` or discovers hooks internally. Hook discovery moves from `renderPages` to `Build()` so the goldmark instance is fully configured before rendering starts. Parsed templates should be cached (parse once, render per node).
...
- `BuildIncremental(cfg, contentMap, previousCache, changedFiles)`: Dev-mode incremental rebuild. Accepts a previous `*cache.Cache` (loaded by the caller, not by this function) and list of changed file paths. Discovers all pages, skips pages where `cache.ShouldSkipFile` returns true (unchanged), and renders pages where it returns false (changed) or that were invalidated via `cache.InvalidatedPages` for layout changes. Returns `BuildResult` with `PagesSkipped` count. When `previousCache` is nil, renders all pages (equivalent to full build). **Disk output (issue #581)**: `BuildIncremental` must write rendered pages to the output directory (same as `Build()`). Extract the page-writing loop from `Build()` into a shared function and call it from both code paths. Only changed pages are written — asset/passthrough copying is handled by existing watches. Integration test in `internal/pipeline/build_test.go` verifies the output file on disk contains updated content after incremental rebuild.
