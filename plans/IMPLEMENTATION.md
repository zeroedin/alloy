...
- **Typed outbound payload structs (issue #529)**: The outbound serialization path (Go → Plugin) must use typed structs with json tags instead of `map[string]interface{}`. This separates the plugin bridge serialization path from the template rendering path (which uses `map[string]interface{}` for liquidgo). Typed structs allow `sonic.ConfigStd` to JIT-compile optimized encoders for fixed fields — only `FrontMatter` and `SiteData` (user-defined keys) require reflection. The inbound path (Plugin → Go) remains `map[string]interface{}` since plugin return shapes are dynamic. Structs are defined in `internal/plugin/payload.go`:
  ...
  - **QuickJS native object fast path (issue #1180)**: The `default` case in `CallHook`'s type switch JSON-serializes the entire payload struct including large HTML/content string fields. For `HookRenderedPayload` (~800KB avg HTML on RHDS), this causes 4 serialization passes per page: `jsonCodec.Marshal` → `JSON.parse` → `JSON.stringify` → `ordered.UnmarshalJSONValue` = ~3.2MB of JSON churn per call. The fix adds type-specific `case` branches that build JS objects directly via the QuickJS API, transferring large string fields as native strings (same zero-serialization path as the old `case string:` branch). Three payload types need fast paths:
    ...
    **Outbound (result extraction)**: The current `hook-call.js` code does `JSON.stringify(__r)` on object results, then Go parses the JSON string back. For the fast path, the developer must change the eval to NOT `JSON.stringify` the entire result when the inbound payload was a fast-path type. Instead: (1) use `result.IsObject()` to detect object returns, (2) extract `html`/`content` via `result.GetPropertyStr("html")` as a native string (zero JSON), (3) extract small auxiliary keys (`addDependencies`, `toc`) via `JSON.stringify` of only those properties, (4) construct the Go result `map[string]interface{}` directly. If `result.IsString()`, handle as before (backward compat for hooks that return raw strings). One approach: use a different eval script for fast-path payloads that returns the raw JS object (not stringified), then extract properties in Go. Another approach: keep the same eval but add a Go-side `jsObjectToGoMap` helper that uses `GetPropertyStr` + `GetOwnPropertyNames` to walk the result without JSON. Either approach must preserve the existing contract: `toGoMap(result)` must succeed with `html` as a string key.
    **Benchmark coverage**: `internal/plugin/wasm_bench_test.go` contains benchmarks at 1KB, 100KB, and 800KB payload sizes plus a string-baseline benchmark for comparison. Baseline numbers (JSON path): 800KB struct = ~34ms/51MB allocs vs 800KB string = ~4.7ms/18MB allocs. The fast path target is within 2x of the string baseline.
    **Test coverage**: 8 spec tests in `internal/plugin/wasm_test.go` under "CallHook payload struct fast path (issue #1180)": field delivery for all three payload types, JSON-special character preservation, empty frontMatter, nested frontMatter objects/arrays, addDependencies extraction, identity return, and large HTML integrity.
