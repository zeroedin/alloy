...
- **File-derived virtual page dependency tracking (issue #1058)**: Optimizes the #970 naive approach (re-render all virtual pages on every incremental rebuild) by making virtual page re-rendering selective based on declared file dependencies. The developer must implement:
  ...
  2. **virtualPageFromMap** (`internal/pipeline/hooks.go`): After constructing the page from the plugin-returned map, read the `dependencies` field. If the key is absent → `page.Dependencies` remains `nil` (safe fallback). If the value fails type assertion to `[]interface{}` (non-array) → log a warning and leave `page.Dependencies` as `nil` (safe fallback). If it succeeds: iterate entries, type-assert each as `string`, collect valid strings into `deps []string`. After iteration: if the original `[]interface{}` was empty → assign `page.Dependencies = []string{}` (tracked, empty). If the original was non-empty but `deps` is empty (all entries were non-string) → leave `page.Dependencies` as `nil` (safe fallback — malformed entries must not produce "never rebuild" semantics). Otherwise → assign `page.Dependencies = deps`.
