...
- `LoadWithDefaults`: Call `Load`, then apply defaults: `Build.Output="_site"`, `Templates.Engine="liquid"`, `Content.Markdown.Goldmark.TemplateTags=true`, `Pagination.Path="page"`, `Plugins.Timeout=5000`, `Content.Formats=["md","html"]`, `Language="en"`, `Build.Clean=true`, `Structure.Content="content"`, `Structure.Layouts="layouts"`, `Structure.Assets="assets"`, `Structure.Static="static"`, `Structure.Data="data"`, `Structure.Plugins="plugins"`. `Content.Markdown.TOC` uses `*bool` tri-state — nil (omitted) defaults to true via `TOCValue()`, explicit `false` is preserved. This is separate from goldmark options because TOC extraction is an Alloy-level concern (AST walk after goldmark parsing), not a goldmark parser option (issue #828).
...
- **Shortcode calling convention (issue #981)**: Two changes to `internal/template/liquid.go`:
  1. **Variable argument resolution**: `parseTagArgs` currently returns `[]string` without distinguishing quoted from unquoted tokens. Refactor to return a `[]tagArg` (or equivalent) where each element carries `value string, quoted bool`. In `alloyInlineTag.Render` and `alloyBlockTag.Render`, iterate the parsed tokens: for quoted args, use the literal value; for unquoted args, call `context.Evaluate(token.value)` (the `liquid.TagContext` interface already provides `Evaluate(expr interface{}) interface{}`). If `Evaluate` returns `nil` (variable not found), fall back to the literal token string. Convert non-string resolved values to strings via `fmt.Sprint()`. The resulting `[]string` is passed to `t.fn(args, content)` as before. The `TagFunc` signature is unchanged. Go engine is unaffected — Go's `html/template` already resolves expressions before calling FuncMap functions.
  2. **Empty return placeholder removal**: In both `alloyInlineTag.Render` and `alloyBlockTag.Render`, remove the `if result == ""` branch that emits `<alloy-shortcode data-tag="%s"></alloy-shortcode>`. When `t.fn` returns `""`, the `Render` method must return `""` — no placeholder, no wrapper element. The Go engine already returns empty string for empty shortcode output, so this change achieves cross-engine parity.
...
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins. Seven categories:
  ...
  - **onConfig merge-back (issue #973)**: The `Build()` call site currently discards the `onConfig` return value (`_, err := hooks.RunWithTimeout(plugin.OnConfig, cfg)`). The developer must:
    ...
    2. Create a function `applyOnConfigResult(cfg *config.Config, result interface{}) error` in `internal/pipeline/build.go` (or a new `config_apply.go` in the same package). This function:
       ...
       - **Path-containment validation (issue #998)**: Before writing each directory path string field (`build.output`, `structure.content`, `structure.layouts`, `structure.assets`, `structure.static`, `structure.data`) to `cfg`, validate the value against path traversal. Create a helper function `validateOnConfigPath(field, value string) (string, error)` that: (1) normalizes the value via `filepath.Clean`, (2) rejects absolute paths via `filepath.IsAbs` with error `"onConfig: <field> must be a relative path, got absolute path <value>"`, (3) rejects paths that start with `".."` (i.e., `cleaned == ".."` or `strings.HasPrefix(cleaned, ".."+string(filepath.Separator))`) with error `"onConfig: <field> <value> traverses above project root"`, (4) returns the cleaned value on success. The cleaned value (not the raw plugin value) is written to `cfg`. This ensures no downstream code (`resolveDir`, `validateOutputDir`, `CleanOutputDir`, `CopyPassthrough`) ever sees an unsafe or denormalized path. Non-path fields (`build.clean`, `passthrough`, `plugins.workers`, `plugins.timeout`) are not validated.
