...
- **Hook payload contract (issue #182)**: All hook payloads must be JSON-serializable for JS/WASM plugins. Seven categories:
  ...
  - **`onFormatRendered`** (issue #1102): fires once per non-HTML format body after template rendering. The developer must:
    ...
    4. **Build helper functions**: Create `buildFormatRenderedPayload(page *content.Page, format string, content string) HookFormatRenderedPayload` in `internal/pipeline/hooks.go` — constructs the typed payload with `convertOrderedMaps(page.FrontMatter)`. Create `extractFormatRenderedContent(result interface{}) (string, bool)` — mirrors `extractPageRenderedHTML` but extracts the `"content"` key instead of `"html"`.
    5. **Pipeline integration in `Build()`**: After the `onPageRendered` dispatch block, add an `onFormatRendered` dispatch block. Iterate all pages: for each page, collect format bodies to dispatch — entries from `page.FormatBodies` (multi-format pages), plus the primary `page.RenderedBody` when `page.Outputs` is non-empty and does not contain `"html"` (single non-HTML pages, using `page.Outputs[0]` as the format string). For each format body, build a `HookFormatRenderedPayload` and dispatch through `ps.Hooks.RunBatchWithProgress(plugin.OnFormatRendered, ...)` or per-item dispatch. Apply back: if `extractFormatRenderedContent` returns a string, replace `page.FormatBodies[format]` (or `page.SetRenderedBody` for single non-HTML pages).
