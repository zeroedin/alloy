...
- `LoadWithDefaults`: Call `Load`, then apply defaults: `Build.Output="_site"`, `Templates.Engine="liquid"`, `Content.Markdown.Goldmark.TemplateTags=true`, `Pagination.Path="page"`, `Plugins.Timeout=5000`, `Content.Formats=["md","html"]`, `Language="en"`, `Build.Clean=true`, `Structure.Content="content"`, `Structure.Layouts="layouts"`, `Structure.Assets="assets"`, `Structure.Static="static"`, `Structure.Data="data"`, `Structure.Plugins="plugins"`
...
- `ResolveLayout`: Lookup chain per spec (front matter > section name > filename > default). Handle `layout: false`. **Bare-extension fallback (issue #827)**: When engine is `"liquid"`, each automatic candidate (post, section, filename, default) must try `<name>.liquid` first, then `<name>.html` (per-candidate interleaving, not global). Explicit front matter `layout:` names are deliberate — try only `<name>.liquid`, and if missing, return an error immediately (no bare-extension fallback, no fall-through to auto candidates). Implementation: split the candidate list into two phases — first check the explicit front matter name (`.liquid` only, error if not found), then build the auto candidates with per-candidate interleaving (`name.liquid`, `name.html` pairs). The Go engine is unchanged — single extension, no fallback.
- `ResolveLayoutWithCascade(page, layoutsDir, engine, permalinkCfg, cascadeData)`: Same lookup chain as `ResolveLayout`, but also considers `_data.yaml` cascade data for layout resolution. Front matter takes priority over cascade data. **Explicit cascade layout names are deliberate** — same hard-error semantics as front matter: try only `.liquid`, error if missing (issue #827). **Implementation note**: the current code has two structurally identical short-circuit paths (front matter at ~line 265, cascade at ~line 271) that both return a path without disk validation. Both must be fixed together to add existence checking and error on missing `.liquid` — fixing one and missing the other would leave inconsistent behavior.
...
- `ResolveLayoutChain`: **Bare-extension fallback (issue #827)**: When resolving parent layouts referenced by `layout:` directives in layout front matter, try `<parent>.liquid` first, then `<parent>.html`.
