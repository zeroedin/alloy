...
- **Typed outbound payload structs (issue #529)**: The outbound serialization path (Go → Plugin) must use typed structs with json tags instead of `map[string]interface{}`. This separates the plugin bridge serialization path from the template rendering path (which uses `map[string]interface{}` for liquidgo). Typed structs allow `sonic.ConfigStd` to JIT-compile optimized encoders for fixed fields — only `FrontMatter` and `SiteData` (user-defined keys) require reflection. The inbound path (Plugin → Go) remains `map[string]interface{}` since plugin return shapes are dynamic. Structs are defined in `internal/plugin/payload.go`:
  ...
  - **Hook chain `map[string]interface{}` fast path (issues #1185, #1188)**: When hooks chain (first hook's result feeds second hook), the payload is `map[string]interface{}` not a typed struct. Add a `case map[string]interface{}:` branch to the `callHookLocked` type switch, guarded by `html` or `content` key presence. For page-like maps: build a JS object directly via `NewObject()`/`SetPropertyStr()` — same approach as the typed struct cases. Maps with `html` key are disambiguated by `toc` presence: `toc` present → `payloadTransform`, otherwise → `payloadRendered`. Maps with `content` key → `payloadFormatRendered`. For non-page maps (no `html`/`content` key, e.g., `onBuildComplete`, `onDataFetched`): fall through to the `default` JSON serialization path. Also add the same guard to `splitBodyField` in `node.go` for the Node IPC split-body path: `map[string]interface{}` with `html` or `content` → split-body framing; without → standard JSON framing.
    **Test coverage**: 7 spec tests — QuickJS: chain receives html from first hook's map result, chain receives content from first hook's map result (onFormatRendered scenario — verifies `content` key guard), non-page map falls through to JSON path, onDataFetched map payload NOT caught by page-like fast path (issue #1188). Node IPC: standard single-header framing for hook map without html/content keys (issue #1188). Pipeline end-to-end: two onPageRendered hooks chain through build with both modifications in output, two hooks chain with large HTML through build without truncation or corruption (issue #1188).
