...
- **Typed outbound payload structs (issue #529)**: The outbound serialization path (Go → Plugin) must use typed structs with json tags instead of `map[string]interface{}`. This separates the plugin bridge serialization path from the template rendering path (which uses `map[string]interface{}` for liquidgo). Typed structs allow `sonic.ConfigStd` to JIT-compile optimized encoders for fixed fields — only `FrontMatter` and `SiteData` (user-defined keys) require reflection. The inbound path (Plugin → Go) remains `map[string]interface{}` since plugin return shapes are dynamic. Structs are defined in `internal/plugin/payload.go`:
  ...
  - **Lazy frontMatter via `Object.defineProperty` getter (issue #1185)**: Replace eager `jsonCodec.Marshal` + `ParseJSON` in `setPayloadFrontMatter` with a lazy getter. Implementation:
    1. Store the Go `map[string]interface{}` in `r.pendingFM` (new field on `QuickJSRuntime`)
    2. During `Init()`, compile `__installLazyFM` JS function that calls `Object.defineProperty(target, 'frontMatter', { get: function() { ... }, set: function(v) { ... }, configurable: true })`
    3. The getter invokes a Go callback `__resolveFM` that marshals `r.pendingFM` and returns the parsed JS object, then replaces the accessor with the resolved data property (so subsequent reads skip the callback)
    4. The setter replaces the accessor with a plain data property set to the written value (no-op that prevents TypeError for `page.frontMatter = {...}`)
    **Test coverage**: 3 spec tests — plugin reads frontMatter.title (correct data), plugin writes frontMatter before reading (no TypeError), plugin never reads frontMatter (no error, no unnecessary marshal).
  - **Lazy TOC via same `Object.defineProperty` pattern (issue #1185)**: Same as lazy frontMatter but for the `toc` field on `HookTransformPayload`. Store in `r.pendingTOC`, install via `__installLazyTOC`, resolve via `__resolveTOC` callback.
    **Test coverage**: 3 spec tests — plugin reads toc (correct entries), plugin writes toc before reading (no TypeError), plugin never reads toc (no error).
