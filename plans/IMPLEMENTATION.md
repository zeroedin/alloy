...
- **Data reload edge cases (issue #719)**: The data reload path has four edge cases requiring test coverage: (1) **Malformed data file**: when `loadSiteData` fails (e.g., malformed JSON), the implementation must preserve the existing `ps.SiteData` rather than replacing it with nil — `loadSiteData` currently returns `(nil, nil)` for parse errors (swallowed in the warning log), causing the `else` branch to overwrite `ps.SiteData` with nil and losing stale data. Pages then re-render with empty data instead of gracefully falling back to the previous values. (2) **Collections-based pagination**: pages with `pagination.data: "collections.posts"` must NOT be invalidated by data file changes — only `site.data.*` sources are affected. (3) **Non-string pagination.data**: the type assertion chain (`pagination.data` → string → `HasPrefix`) must silently skip non-string values without panic. (4) **Custom data directory**: the `dataPrefix` check must use `cfg.Structure.Data` (configured value), not fall back to `"data"` when the user has configured a custom path.
