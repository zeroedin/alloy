...
- **WASM packed i64 ABI (issue #1222)**: All data-returning WASM exports use a packed `i64` return convention: `result = (ptr << 32) | len`. The host unpacks as `ptr = uint32(result >> 32)`, `len = uint32(result & 0xFFFFFFFF)`. This replaces the previous multi-value `(result i32 i32)` form, which is not producible by Rust ≥ 1.82, TinyGo, or AssemblyScript without unstable/unsupported compiler flags. The packed form is the natural output of all three toolchains (`-> u64` in Rust, `uint64` in TinyGo, `u64` in AssemblyScript).
  - **Call sites to update**: Four functions in `wasm.go` read `(ptr, len)` pairs from WASM results and must change from `results[0]`/`results[1]` two-value reads to packed `i64` unpacking:
  ...
  - **Test fixtures (issue #1222)**: All 11 WAT fixtures in `testdata/single-files/` must be updated from `(result i32 i32)` to `(result i64)` with the packed shift/or return sequence. Two new WAT fixtures must be added: `wasm-old-multivalue-abi.wat/.wasm` (uses the old `(result i32 i32)` form, for testing load-time rejection) and `wasm-sret-abi.wat/.wasm` (uses the sret `(param i32 i32 i32) -> ()` form, for testing load-time rejection). All WAT sources are compiled with `wat2wasm` — no toolchain dependency in CI.
  - **Test coverage (issue #1222)**: Spec tests in `internal/plugin/wasm_test.go` under "WASM packed i64 ABI (issue #1222)": packed filter returns transformed string, packed filter is not passthrough (disambiguation), packed error convention (packed `0` returns error), packed `last_error()` returns error message, packed `hooks()` discovers hook names, packed `hook()` processes payload, packed `CallExportRaw` works, `CallExport` with zero args returns error, load-time rejection of old multi-value `(result i32 i32)` ABI with diagnostic, load-time rejection of sret `(param i32 i32 i32)` ABI with diagnostic, and at least one end-to-end pipeline test exercising a packed-ABI WASM plugin through the build.
