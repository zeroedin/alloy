...
15. **Plugin Hook: `onPageRendered`** — Plugins can post-process intermediate page HTML
...
- **Asset and static copy** starts in a background goroutine after ALL validation completes and the output directory is created/cleaned (issue #492, #503). The goroutine must start after: `onConfig` (plugins mutate source dirs), `onBeforeValidation` (plugins add output paths), conflict detection (validates no overlaps), and `onAfterValidation` (plugins inspect final manifest). Starting before validation completes means writing to the output directory before the build is confirmed valid — a validation failure would leave partial copies as debris. The goroutine runs concurrently with pipeline init, content discovery, rendering, and layout processing. The build waits for the goroutine before returning. No conflict with output writing since static files and rendered pages target different paths.
...
4. Pre-render computed data  (onContentLoaded plugins — before Markdown)
5. Post-render computed data (onContentTransformed plugins — after Markdown, before Liquid)
```
