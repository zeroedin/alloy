...
1. Detect and load config file (via `config.DetectConfigFile` + `config.LoadWithDefaults`). **If no config file is found, exit 1 with an actionable error** (issue #1239). The error message must include: the directory that was searched, the expected file names (`alloy.config.{yaml,yml,toml,json}`), and a suggestion to use `--config` for non-standard layouts. When `--config` was explicitly passed and the file does not exist, the error must mention the specified path and must NOT suggest `--config` (circular advice). A config file is always required for `build`, `dev`, and `serve` — CLI flags are operational overrides, not a config replacement.
