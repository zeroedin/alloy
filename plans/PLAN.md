...
**Explicit collections** — A section listed in the `collections:` config becomes a collection regardless of its permalink pattern. This allows non-blog sections (e.g., `releases/`, `changelog/`, `projects/`) to participate in pagination and template loops without requiring date-based URL structure. If a section qualifies both via date tokens and explicit config, it produces a single collection (no duplication). Explicit collections follow all the same rules as date-token collections: section index files are excluded, drafts are excluded in build/serve mode, lifecycle filtering applies.
