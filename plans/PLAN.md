...
- **Timeout**: Each plugin hook and source call has a configurable timeout (default 5s, `cfg.Plugins.Timeout`). If exceeded, build continues without that plugin's modifications and logs a warning. Hook functions receive a `context.Context` as their first parameter carrying the timeout deadline. Hooks should check `ctx.Done()` for cooperative cancellation — a timed-out hook's goroutine will not be forcefully killed, but the context signals that its result will be discarded. Source calls (`CallSource`) must apply the same timeout — a slow source handler (DNS timeout, unresponsive CMS API) must not hang the build indefinitely.
