...
- `__installLazyFM(target, jsonStr)` — installs a lazy getter on the `frontMatter` property via `Object.defineProperty`. Defers `JSON.parse(jsonStr)` until the plugin first accesses `page.frontMatter`. The getter is memoized (returns the same parsed object on repeated access). The setter allows `page.frontMatter = {...}` to override the lazy value. The property must be `enumerable: true` and `configurable: true` so `Object.keys(page)` and `JSON.stringify(page)` include it.
- `__installLazyTOC(target, jsonStr)` — same pattern for the `toc` property on `HookTransformPayload` objects. Defers `JSON.parse` of the TOC array until accessed. Must be `enumerable: true` and `configurable: true`.
