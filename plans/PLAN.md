...
| 6 | **Auto-generated files** — sitemap.xml, when `sitemap.enabled` | `sitemap.xml (generated)` |
...
- **`onAfterValidation`** — Plugins receive the validated output manifest and the site data cascade. Payload: `{ outputPaths: ["about/index.html", ...], cascade: { ...site data... } }`. `outputPaths` is read-only (includes plugin-added paths from `onBeforeValidation`). `cascade` is mutable — changes are applied back to site data. Return: `{ cascade: { ...modified data... } }` — merged into `siteData`. Changes to `outputPaths` in the return are ignored. Return without `cascade` key is a no-op (observation only). Unrecognized return keys produce an error (even alongside recognized keys like `cascade`). Fires immediately after conflict detection passes. This is the point where plugins can inspect what pages will be built, inject data into the cascade for templates, compute derived values, or validate the merged dataset. The cascade is trustworthy here — validation has passed, all pages are confirmed valid.
...
- **Plugin filter or shortcode call error during rendering aborts the build (issue #1224).** When a template invokes a plugin-registered filter or shortcode and the call returns an error (JS throw, WASM trap, Node error), the error must propagate as a render failure for the page — no silent fallback to original input (filters) or empty string (shortcodes). This applies to all plugin tiers (QuickJS, WASM, Node) and both template engines (Liquid, Go templates). The error message must include the filter/shortcode name and the page path. Silent passthrough on filter errors is the most dangerous failure mode — code that loads fine, compiles, and silently does nothing, producing incorrect output with no warning.
...
- **Plugin filter or shortcode call error shows error overlay (issue #1224).** Same as any page render failure — the error overlay displays the filter/shortcode name, page path, and error message. Other pages continue to serve. The error propagation mechanism is the same as in build mode (filter/shortcode errors surface as render errors), but the handling differs: page-level errors are contained rather than aborting the server.
...
**`Page.SetRenderedHTML(s string)` (issues #1185, #1189)**: When applying hook results back to page state, the pipeline has a Go string (from QuickJS `GetPropertyStr().String()` or JSON unmarshal). `SetRenderedBody([]byte(html))` converts string→`[]byte` (allocation), clears the cached `renderedStr`, and then the next `HTML()` call converts `[]byte`→string again (second allocation). `SetRenderedHTML(s)` stores both representations simultaneously: `RenderedBody = []byte(s)` and `renderedStr = s`. The `[]byte` conversion still happens (needed for file writers that read `RenderedBody`), but the cached string survives so `HTML()` returns it without re-conversion. `SetRenderedBody` must still clear the cached string (existing behavior) so code paths that set raw `[]byte` don't serve stale `renderedStr`. The pipeline uses `SetRenderedHTML` in hook result application sites (`fireContentTransformedHooks`, `dispatchPostRenderHooks`) where the source is already a string.
...
**Pre-compiled JS helpers (issue #1186)**: `invokeHookFastPath` calls `ctx.Eval("__hooks[__callHookName](__callInput)")` per page per hook, re-parsing and re-compiling the same static JS expression on every invocation. `setPayloadFrontMatter` and the inline TOC setup in `callHookTransformPayload` eagerly JSON-parse metadata that the plugin may never access. The fix pre-compiles three named JS functions during `Init()` and invokes them via `ctx.Invoke()` (direct function call, no parse/compile):
- `__callHookByName(name, input)` — replaces the `Eval` in `invokeHookFastPath`. Receives hook name and input as parameters instead of reading globals.
- `__installLazyFM(target, jsonStr)` — installs a lazy getter on the `frontMatter` property via `Object.defineProperty`. Defers `JSON.parse(jsonStr)` until the plugin first accesses `page.frontMatter`. The getter is memoized (returns the same parsed object on repeated access) using a unique sentinel object (not `null`) to distinguish "not yet parsed" from "explicitly set to null" via the setter. The setter allows `page.frontMatter = {...}` or `page.frontMatter = null` to override the lazy value. The property must be `enumerable: true` and `configurable: true` so `Object.keys(page)` and `JSON.stringify(page)` include it.
- `__installLazyTOC(target, jsonStr)` — same pattern for the `toc` property on `HookTransformPayload` objects. Defers `JSON.parse` of the TOC array until accessed. Uses the same unique sentinel pattern. Must be `enumerable: true` and `configurable: true`.

Nil `frontMatter` continues to be set as an empty JS object directly (no lazy getter needed — there is nothing to defer). Nil or empty `toc` is omitted from the object (matching existing `omitempty` behavior). The lazy getter pattern is transparent to plugins — they access `page.frontMatter.title` and `page.toc[0].text` identically. See IMPLEMENTATION.md §Plugin System for detailed guidance.

**Hook chain fast path for `map[string]interface{}` payloads (issues #1188, #1202)**: When hooks chain (hook 1's result feeds hook 2), the payload for hook 2 is `map[string]interface{}` — not a typed struct like `HookRenderedPayload`. The `callHookLocked` type switch must include a `case map[string]interface{}:` branch that uses the **hook name** to determine the payload type: `"onPageRendered"` → `payloadRendered`, `"onFormatRendered"` → `payloadFormatRendered`, `"onContentTransformed"` → `payloadTransform`. Maps for unknown hook names fall through to the default JSON serialization path. The hook name is always available as the `name` parameter and unambiguously identifies the correct extractor — unlike the previous key-content heuristic (`html`/`toc`/`content` presence), which fails when a plugin returns extra keys (e.g., `onPageRendered` returning `{html, toc}` triggers misclassification as `payloadTransform`; issue #1202). The Node IPC path (`splitBodyField` in `EncodeMessage`) retains key-based detection (unchanged by this refactor): `map[string]interface{}` payloads with `html` or `content` use split-body framing; maps without those keys use standard single-header JSON framing. Test coverage: 11 spec tests (4 QuickJS chain maps (including onDataFetched guard), 1 Node IPC standard framing for non-page maps, 2 pipeline end-to-end with chained hooks, plus 4 hook name dispatch disambiguation tests — issues #1202, #1211, #1212).
...
**Partials and includes** — both engines resolve partials from the layouts directory:
- **Liquid:** `{% include "partial" %}` and `{% render "partial" %}` for partials. Resolves from the layouts directory, trying `name.liquid`, then `name.html`. Extensionless files are not resolved — a template file without a recognized extension has no clear use case and is almost certainly a mistake (issue #880). **Plugin-registered filters must work in partials** — the same filter dispatch mechanism that applies to content templates and layouts must also apply to included files. If the engine uses template pre-processing (e.g., rewriting novel filter names for dispatch), that pre-processing must also run on partial source before parsing (issue #376).
- **Go:** `{{ include "path" }}` function for includes (issues #823, #883). Named `include` for parity with Liquid's `{% include %}`. Resolves from the layouts directory, trying `path.html` only. Extensionless files are not resolved (issue #880). The context argument is optional — defaults to the current template dot (like Liquid's `{% include %}` which shares scope). Explicit context narrows scope: `{{ include "partials/card" (dict "item" . "compact" true) }}`. Returns rendered output as `template.HTML` (usable in variable assignment and pipelines — unlike `{{ template }}` which is an action, not a function). `{{ block "name" . }}` / `{{ define "name" }}` remain available for same-file layout inheritance. Nesting depth is capped at 100 (matching Ruby/Go Liquid's `Block::MAX_DEPTH`); exceeding it is a build error. Missing includes and paths that traverse outside the layouts root are build errors.
...
Extension matching is case-insensitive — `.SVG`, `.Png`, etc. are normalized via `strings.ToLower` before checking the text-type allowlist (issue #931).
...
**Pipeline integration**: In `renderPages()`, when the configured engine is `"gotemplate"`, the preprocessor must be called on the post-Goldmark HTML before template parsing. The callback routes through the plugin registry's `CallShortcode`. This step runs after `escapeTemplateTagsInCode` and before `hasTemplateSyntax`/template rendering. Block shortcodes and Go template `{{ }}` expressions coexist in the same page — the preprocessor replaces `{{% %}}` tags first, then the Go template engine evaluates remaining `{{ }}` expressions (including any `{{ }}` expressions that appeared inside block shortcode content). Without this wiring, `ProcessBlockShortcodes` is dead code and users get a Go template parse error (`unexpected "%" in command`) when using `{{% %}}` syntax.
...
**Escaped quotes in arguments**: Quoted arguments support backslash escaping. `\"` inside a quoted argument produces a literal `"` in the extracted argument value. `\\` produces a literal `\`. For example, `{{% tag "she said \"hello\"" %}}` passes `she said "hello"` as the argument. Arguments without escape sequences are unaffected — existing behavior is preserved.

**Depth guard**: The preprocessor limits total iterations to prevent infinite loops from self-referential shortcode output. If a callback returns output that contains new `{{% %}}` block shortcode pairs, those are processed in subsequent iterations. If the total iteration count exceeds the limit, a build error is returned. This protects against callback output that recursively generates shortcodes (e.g., a shortcode whose output contains another invocation of itself).

**Error handling**: An unclosed `{{% tag %}}` (no matching `{{% /tag %}}`) is a build error. A mismatched closing tag name is a build error. Interleaved tags (`{{% a %}}{{% b %}}{{% /a %}}{{% /b %}}`) are a build error — the error references both the expected and actual tag names. Error messages include the tag name. Plugin callback errors propagate as build errors. When the opening tag of a pair is inside a `<code>` element but the closing tag is outside, the opening tag is treated as literal text and the closing tag produces an "unexpected closing tag" error.
...
**Markdown-only feature.** `.html` content files never enter the Goldmark stage, so shortcode bodies in `.html` files already reach the shortcode function unmodified. The `>` modifier is therefore meaningless — not merely unsupported — in `.html` files, and needs no code-level validation or special-casing there: a `{%>` written in an `.html` file reaches the engine verbatim and produces the engine's native parse error. This must be documented as a Markdown-only concept.

This rests on the **Markdown-only Goldmark invariant** (issue #1249), specified under Phase 1 below. The raw block modifier's `.html` contract is only as strong as that invariant: any pipeline path that hands an `.html` body to Goldmark would strip the marker and could hard-fail the build on an unterminated block, both of which this section explicitly says must not happen.
...
Alloy scans `plugins/` for `.js` and `.ts` files that export `runtime: "node"`. If any are found, it spawns subprocess workers that load all of them. If none are found, no Node process is started — zero overhead. Communicates via stdin/stdout using length-prefixed JSON-RPC (LSP-style framing). Plugin stderr is connected to Alloy's own stderr — plugin `console.log`, `console.warn`, and `process.stdout.write` (patched to stderr by the bridge) output appears in the user's terminal alongside Alloy's build output. No log file is created.
...
**Stdout isolation (issue #968)**: The bridge script patches `process.stdout.write` at startup, before any plugin code runs, to redirect all writes to `process.stderr`. The bridge holds the only reference to the real `process.stdout.write` and uses it exclusively for sending protocol frames via `sendMessage`. This prevents high-level stdout and console writes from plugin code — including npm dependencies that call `process.stdout.write` directly (progress bars, debug logging, etc.) — from injecting non-protocol bytes into the stdout channel and desyncing the frame reader. The existing `console.log/warn/info/debug` patches (which redirect to `process.stderr.write`) remain in place for formatting convenience but are no longer the sole line of defense. **Known limitation**: child processes spawned by a plugin with inherited stdio can still write to the real stdout fd. In-process code can also bypass the patch via fd-level writes — `require('fs').writeSync(1, 'junk')` or `new net.Socket({fd:1}).write(...)` — since the patch only intercepts the high-level `process.stdout.write` method. Both vectors share the same root cause (fd-level writes bypass the JS-level patch) and the same fix: a channel change to Unix sockets / named pipes, tracked under #490.
...
| `onBeforeValidation` | `{ outputPaths: ["about/index.html", "blog/index.html", ...] }` | `{ addOutputs: { "path": "source" } }` | Before conflict detection. Plugin adds output paths via `addOutputs` map. Added paths feed into `DetectConflicts()`. Return without `addOutputs` is no-op. Unrecognized keys produce error even alongside `addOutputs`. |
| `onAfterValidation` | `{ outputPaths: [...], cascade: { ... } }` | `{ cascade: { ... } }` | After validation passes. Plugin inspects output manifest and injects cascade data. `outputPaths` is read-only (changes ignored). `cascade` changes merged into `siteData`. Return without `cascade` is no-op. Unrecognized keys produce error even alongside `cascade`. |
...
**Processing:** `ParseFileChangedResult` extracts the structured return value. Non-map returns (nil, string, boolean) are treated as no-op for backward compatibility. Unrecognized keys are ignored (forward compatible). Non-array `invalidateByDependency` produces a warning and is treated as nil. Non-string entries within the array are filtered out with a warning. Non-boolean `restart` values (numeric, string, nil) default to `false` with a warning — JS-truthy values like `1` or `"yes"` are not valid boolean restart values in Go. When a recognized key is present but malformed (e.g., `restart: 1` as the sole key), the result is non-nil with a warning, not nil — returning nil would mean "no recognized keys found" which is incorrect.
...
- **Timeout**: Each plugin hook and source call has a configurable timeout (default 5s, `cfg.Plugins.Timeout`). Hook functions receive a `context.Context` as their first parameter carrying the timeout deadline. Hooks should check `ctx.Done()` for cooperative cancellation — a timed-out hook's goroutine will not be forcefully killed, but the context signals that its result will be discarded. Hook timeouts discard that plugin's modifications, log a warning, and let the build continue. Source call timeouts (`CallSource`) have different semantics: a timed-out source returns a fetch error that aborts the build (per §5 — source failures are fatal). A slow source handler (DNS timeout, unresponsive CMS API) must not hang the build indefinitely.
- **Batching**: Pages are sent to Node plugins in batches (not one-at-a-time) to amortize IPC overhead. WASM plugins process in-process so batching is unnecessary for IPC amortization, but QuickJS uses `BatchCallHook` for goroutine-free synchronous dispatch — a simple loop calling `CallHook` internally without per-item goroutine/channel/context overhead (issue #1190). On error, `BatchCallHook` wraps with `fmt.Errorf("item %d: %w", i, err)` to identify which payload (0-based index) failed. The existing `registerRuntime` batch detection (type assertion for `BatchCallHook` method) routes `RunBatchWithProgress` through this single-goroutine path. **Zero-item guard (issue #972)**: When a batch hook is called with 0 items (e.g., all pages were filtered by scope or skipped during incremental rebuild), the hook is skipped entirely — no timeout calculation, no subprocess dispatch, no warning. Without this guard, the effective timeout is `timeout × 0 = 0ms`, which expires instantly and produces a spurious warning.
...
**Markdown-only Goldmark invariant (issue #1249).** No pipeline path may hand an `.html` (or `.txt`) content body to Goldmark. Format detection is what decides, and each non-Markdown format has its own contract: Goldmark runs for `.md` only; `.html` bodies reach the template engine exactly as authored; `.txt` bodies go through `RenderText`, which wraps them in `<pre>` and HTML-escapes them. `RenderText` is a transformation, but not a Markdown one — `&` and `<` are escaped there while headings, emphasis, and typographer substitutions are not applied, so `.txt` and `.html` must be asserted separately rather than under one "passes through untouched" claim. This is a whole-pipeline invariant, not a property of one function, because features specified against it inherit its blast radius: the raw block shortcode `>` modifier (§Shortcodes) is defined as meaningless in `.html` precisely because no Goldmark stage exists there to interpret it. A path that violates the invariant silently gives `.html` files Markdown semantics — entity escaping, typographer substitution, `unsafe` filtering, custom-element block parsing, raw-marker stripping, and Markdown-stage build errors — that the rest of the spec says they do not have.

There is exactly one production Markdown entry point in the pipeline, and it must stay that way:

| Entry point | Contract |
| --- | --- |
| `renderPages` (`internal/pipeline/render.go`) | The only pipeline path that renders content. Routes on extension: `.md` → Goldmark, `.txt` → `RenderText`, everything else → body passthrough. |
| `markdownify` filter (`internal/template/filters.go`) | Deliberately outside the invariant. It is an explicit author-invoked filter over a value, not a content file, and forces `TemplateTags: false`. Its input is a string the author chose to render as Markdown, whatever the containing page's extension. |

**SSR Phase 1 is `renderPages`.** Phase 1 has one implementation. `Build()` and `BuildIncremental()` render through `renderPages` and collect the results into the intermediate-HTML map that `BuildPhase2` consumes; SSR does not get a second, simplified renderer of its own.

**`BuildPhase1` is removed** (issue #1249). It was a test-only reimplementation of Phase 1 in `internal/pipeline/ssr.go` that drifted from `renderPages`: it ran Goldmark on every discovered page regardless of extension — violating this invariant — and never rendered templates or applied layouts, so it did not produce Phase 1 output in the first place. It had no production caller; `Build()` and `BuildIncremental()` both go through `renderPages`. Its two `ssr_test.go` specs exercise the Phase 1 → Phase 2 handoff through the real path instead. Adding the missing content-type switch was rejected as a fix: it would have left a second Phase 1 free to drift again, and the `.html` divergence it produced was the second such drift, not the first.
...
| `render-codeblock` | `inner` (content-escaped code text — angle brackets and ampersands are entity-encoded so HTML displays as text, not markup; quotes are NOT escaped because codeblock inner is element content, not attribute context — issue #962), `language` (HTML-escaped via `html.EscapeString` — language goes into attributes where quote escaping is correct), `attributes` |
...
**Template tag escaping** — The pipeline's `escapeTemplateTagsInCode` step protects `{{ }}`/`{% %}` inside `<code>` elements from Liquid processing. When a render hook replaces `<pre><code>` with a different structure (e.g., `<rh-code-block><script>`), `escapeTemplateTagsInCode` cannot find `<code>` elements to protect. Instead, codeblock render hooks apply two escaping steps to `markup.inner` before passing it to the hook template: (1) Content-only HTML escaping via `escapeHTMLText` — converts `&`, `<`, `>` to entities so HTML tags in code display as text, not markup (issue #947). Quotes (`"`, `'`) are NOT escaped because codeblock `inner` is element content, not attribute context — escaping them to `&#34;`/`&#39;` breaks downstream consumers (e.g., shiki's `decodeHtmlEntities`) that do not decode quote entities (issue #962). (2) Liquid delimiter escaping via `escapeLiquidDelimiters` — converts `{{ }}` and `{% %}` to HTML entities so the template engine does not execute them. Content-only escaping runs first, then Liquid delimiter escaping. This ensures code content is safe regardless of what HTML structure the hook template produces.

**Codeblock inner escaping contract (issue #962)** — The escaping applied to codeblock `markup.inner` and the decoding performed by downstream consumers (e.g., the shiki plugin's `decodeHtmlEntities`) must stay in sync. The producer (`renderFencedCodeBlock`) encodes exactly these entities: `&amp;`, `&lt;`, `&gt;`, `&#123;`, `&#125;`. The consumer must decode exactly those entities to reconstruct the raw source. Adding new entity encodings to the producer without updating consumers will cause literal entity text to appear in highlighted output. Removing decodings from the consumer without updating the producer will cause incorrect display. Changes to either side must check the other. The `language` field uses full `html.EscapeString` (5 characters) because it lands in an HTML attribute context (`lang="…"`) where quote escaping is correct — this is not part of the inner escaping contract.
...
**Block-level raw HTML in markup.inner (issue #955)** — The `escapingRawHTMLRenderer` registered on the child goldmark renderer escapes inline raw HTML (`ast.KindRawHTML`) but must also handle block-level raw HTML (`ast.KindHTMLBlock`). Block HTML inside blockquote content (e.g., `> <script>alert(1)</script>`) passes through `renderChildrenToHTML` unescaped without this. Headings are not affected — they contain only inline content, so inline raw HTML (`ast.KindRawHTML`) is the relevant node type, already handled by the existing handler. The fix is to register an `ast.KindHTMLBlock` handler on `escapingRawHTMLRenderer` that entity-encodes the block's line segments, matching the inline handler's approach. Goldmark's own formatting elements (`<strong>`, `<em>`, `<a>`, etc.) are unaffected — they are AST nodes rendered by goldmark's standard renderers, not raw HTML nodes.
...
- **Single shared instance per build** (issue #353) — one pipeline goldmark instance is created per build in `Build()`, after `createEngine` returns and after render hook discovery. Stored on `RenderContext.Goldmark` (`goldmark.Markdown`). Fully configured with hooks, TemplateTags, Typographer, Unsafe, AutoHeadingID — everything from `cfg.Content.Markdown`. Hook discovery moves from inside `renderPages` to `Build()` so the goldmark instance is complete before rendering starts. `markdownify` keeps its own separate instance (no hooks, `TemplateTags: false`) initialized in `createEngine` via `InitMarkdownify`. Both instances read base config from `cfg.Content.Markdown` — they diverge on hooks and TemplateTags. `goldmark.Markdown.Convert` is safe for sequential reuse — no mutable state between calls.
- **Consolidated render function** (issues #353, #700) — single function `RenderMarkdown(source []byte, md goldmark.Markdown) ([]byte, []TOCEntry, error)`. No convenience wrappers — the old `RenderMarkdown(source, opts)` and `RenderMarkdownWithTOC(source, opts)` wrappers from #693 are deleted. All callers pass a pre-built goldmark instance via `CreateGoldmark(opts)`. Always returns TOC (the AST heading walk is negligible cost relative to the parse/render). `renderPages` uses it for `page.TOC`; callers that do not need it discard it with `_`. `CreateGoldmark` stays exported for tests and one-off use.
