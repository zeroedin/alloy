...
- **Go:** `{{ partial "path" }}` function for partials (issue #823). Resolves from the layouts directory, trying `path.html`, then the raw name. The context argument is optional — defaults to the current template dot (like Liquid's `{% include %}` which shares scope). Explicit context narrows scope: `{{ partial "partials/card" (dict "item" . "compact" true) }}`. Returns rendered output as `template.HTML` (usable in variable assignment and pipelines — unlike `{{ template }}` which is an action, not a function). `{{ block "name" . }}` / `{{ define "name" }}` remain available for same-file layout inheritance. Nesting depth is capped at 100 (matching Ruby/Go Liquid's `Block::MAX_DEPTH`); exceeding it is a build error. Missing partials and paths that traverse outside the layouts root are build errors.
