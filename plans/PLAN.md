...
| `render-heading` | `inner` (rendered inner HTML), `level` (1-6), `id` (custom `{#id}` attribute if present, otherwise auto-generated slug via `slugify` — e.g., "My Section" → `my-section`), `text` (HTML-escaped plain text), `attributes` (map of goldmark-parsed attributes from `{.class #id key="value"}` syntax) |
