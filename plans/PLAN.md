...
- **Asset and static copy** starts in a background goroutine after `onConfig` fires and the output directory is created/cleaned (issue #492, #503). The goroutine must start AFTER `onConfig` because plugins can mutate `cfg.Structure.Static`, `cfg.Structure.Assets`, `cfg.Passthrough`, and `cfg.Build.Output` — starting before `onConfig` would copy from wrong directories. The goroutine runs concurrently with pipeline init, content discovery, rendering, and layout processing. The build waits for the goroutine before returning. No conflict with output writing since static files and rendered pages target different paths.
