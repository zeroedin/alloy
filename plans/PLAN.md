...
**Markdown-only Goldmark invariant (issue #1249).** No pipeline path may hand an `.html` (or `.txt`) content body to Goldmark. Format detection is what decides — Goldmark runs for `.md` only, and every other format reaches the template engine as authored. This is a whole-pipeline invariant, not a property of one function, because features specified against it inherit its blast radius: the raw block shortcode `>` modifier (§Shortcodes) is defined as meaningless in `.html` precisely because no Goldmark stage exists there to interpret it. A path that violates the invariant silently gives `.html` files Markdown semantics — entity escaping, typographer substitution, `unsafe` filtering, custom-element block parsing, raw-marker stripping, and Markdown-stage build errors — that the rest of the spec says they do not have.
