...
**Hook chain fast path for `map[string]interface{}` payloads (issue #1188)**: When hooks chain (hook 1's result feeds hook 2), the payload for hook 2 is `map[string]interface{}` — not a typed struct like `HookRenderedPayload`. The `callHookLocked` type switch must include a `case map[string]interface{}:` branch that checks for `html` or `content` keys. Maps with `html` key use the rendered/transform fast path (disambiguated by `toc` presence). Maps with `content` key use the format-rendered fast path. Maps without either key (e.g., `onDataFetched`, `onBuildComplete` payloads) fall through to the default JSON serialization path. The key guard is critical — `map[string]interface{}` is used for many hook payloads, not just page results. Without it, non-page maps would hit the fast path and lose fields. The Node IPC path (`splitBodyField` in `EncodeMessage`) applies the same key guard: `map[string]interface{}` payloads with `html` or `content` use split-body framing; maps without those keys use standard single-header JSON framing. Test coverage: 4 spec tests (1 QuickJS onDataFetched guard, 1 Node IPC standard framing for non-page maps, 2 pipeline end-to-end with chained hooks).
