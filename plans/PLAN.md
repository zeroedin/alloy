...
- **Liquid:** `{% include "partial" %}` and `{% render "partial" %}` for partials. Resolves from the layouts directory, trying `name.liquid`, then `name.html`, then the raw name. **Plugin-registered filters must work in partials** — the same filter dispatch mechanism that applies to content templates and layouts must also apply to included files. If the engine uses template pre-processing (e.g., rewriting novel filter names for dispatch), that pre-processing must also run on partial source before parsing (issue #376).
- **Go:** `{{ include "path" }}` function for includes (issues #823, #883). Named `include` for parity with Liquid's `{% include %}`. Resolves from the layouts directory, trying `path.html`, then the raw name. The context argument is optional — defaults to the current template dot (like Liquid's `{% include %}` which shares scope). Inside `{{ range }}` and `{{ with }}` blocks, the include receives the rebound dot (the current loop element or narrowed scope), not the root context — this is ensured by the parse-time dot injection rewrite (issue #884). Explicit context narrows scope: `{{ include "partials/card" (dict "item" . "compact" true) }}`. The `dict` helper requires an even number of arguments; an odd count is a render-time error with a message indicating the requirement (issue #884). Returns rendered output as `template.HTML` (usable in variable assignment and pipelines — unlike `{{ template }}` which is an action, not a function). `{{ block "name" . }}` / `{{ define "name" }}` remain available for same-file layout inheritance. Nesting depth is capped at 100 (matching Ruby/Go Liquid's `Block::MAX_DEPTH`); exceeding it is a build error. Missing includes and paths that traverse outside the layouts root are build errors. Concurrent `Render` calls on the same engine instance are safe — depth tracking (`atomic.Int32`) and include caching (`sync.Map`) are goroutine-safe (issue #884).
